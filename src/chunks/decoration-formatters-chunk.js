const { describeFile } = require('./decorations-static');
...
function buildSummaryTooltip(provider, { filePath, stat, badgeDetails }) {
    try {
        ...
        provider._logger?.debug?.('decoration-formatters: buildSummaryTooltip failed', error);
...
function formatDateReadable(provider, date) {
    try {
        ...
        provider._logger?.debug?.('decoration-formatters: formatDateReadable failed', error);
        try { return provider._l10n.formatDate(date); } catch { return date.toString(); }
    }
}
...
function formatFullDate(provider, date) {
    try {
        ...
        provider._logger?.debug?.('decoration-formatters: formatFullDate failed', error);
...
function formatCompactSize(provider, bytes) {
    try {
        ...
        provider._logger?.debug?.('decoration-formatters: formatCompactSize failed', error);
        return null;
    }
}
...
function getInitials(provider, fullName) {
    try {
        if (provider._gitInsightsManager && typeof provider._gitInsightsManager.getInitials === 'function') return provider._gitInsightsManager.getInitials(fullName);
    } catch (e) { provider._logger?.debug?.('decoration-formatters: delegated getInitials failed', e); }
...
module.exports = { buildSummaryTooltip, formatDateReadable, formatFullDate, formatCompactSize, getInitials };
