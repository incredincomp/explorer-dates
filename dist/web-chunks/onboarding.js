...
${t.description}`;await vscode.window.showInformationMessage(o,"Show More Tips","Got it!")==="Show More Tips"&&await this.showFeatureTour()}async showWhatsNew(e){try{let t=vscode.window.createWebviewPanel("explorerDatesWhatsNew",`Explorer Dates v${e} - What's New`,vscode.ViewColumn.One,{enableScripts:!0,retainContextWhenHidden:!1}),o=await this.M(e);t.webview.html=o,t.webview.onDidReceiveMessage(async s=>{switch(s.command){case"openSettings":await vscode.commands.executeCommand("workbench.action.openSettings","explorerDates"),t.dispose();break;case"tryFeature":s.feature==="badgePriority"&&(await this.o.updateSetting("badgePriority","author",{scope:"user",reason:"whats-new-demo"}),vscode.window.showInformationMessage("Badge priority set to author! You should see author initials on files now."));break;case"dismiss":t.dispose();break}})}catch(t){this._logger.error("Failed to show what's new",t)}}async M(e){try{let{loadOnboardingAssets:t}=b(),o=await t();if(o)return this._logger.debug("Using chunked onboarding assets for what's new"),await o.getWhatsNewHTML(e)}catch(t){this._logger.warn("Failed to load chunked assets for what's new, using inline fallback",t)}return this._(e)}_(e){return`<!DOCTYPE html><html><head><meta charset="utf-8"><title>What's New</title></head><body><h1>What's New ${e}</h1><p>Full content loads on demand.</p></body></html>`}};module.exports={OnboardingManager}});var N=p((de,W)=>{var{OnboardingManager:P}=T();W.exports={OnboardingManager:P,createOnboardingManager:i=>new P(i)}});var R=p((ue,I)=>{var{getLogger:oe}=v(),g=class{constructor(){this._logger=oe(),this.t=null,this.r=!1}async initialize(){if(!this.r)try{this.t={setupWizardTemplate:this.O(),featureTourTemplate:this.A(),whatsNewTemplate:this.F(),sharedStyles:this.l(),sharedScripts:this.d()},this.r=!0,this._logger.debug("Onboarding assets initialized (~23KB lazy loaded)")}catch(e){throw this._logger.error("Failed to initialize onboarding assets",e),e}}async getSetupWizardHTML(e){return await this.initialize(),this.L(this.t.setupWizardTemplate,e)}async getFeatureTourHTML(){return await this.initialize(),this.t.featureTourTemplate}async getWhatsNewHTML(e){return await this.initialize(),this.P(this.t.whatsNewTemplate,e)}L(e,t){let o=Object.entries(t).map(([r,a])=>`
        ...
        `).replace("{{PRESETS_JSON}}",JSON.stringify(t))}P(e,t){return e.replace(/{{VERSION}}/g,t)}O(){return`
            <!DOCTYPE html>
            <html>
            <head>
                <meta charset="UTF-8">
