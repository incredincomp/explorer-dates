...
`),l)try{let g=t||e,N=(await this._fileSystem.readFile(g,"utf8")).split(`
`).length;u+=`\u{1F4CF} Lines: ${N.toLocaleString()}
`}catch{}return u+=`\u{1F4C2} Path: ${e}`,s&&(u+=`

\u{1F464} Last Modified By: ${s.authorName}`,s.authorEmail&&(u+=` (${s.authorEmail})`),s.authorDate&&(u+=`
   ${s.authorDate}`)),u}_formatDateReadable(e){let t=new Date,o=t.getTime()-e.getTime(),i=Math.floor(o/(1e3*60)),s=Math.floor(o/(1e3*60*60)),r=Math.floor(o/(1e3*60*60*24));return i<1?this._l10n.getString("justNow"):i<60?this._l10n.getString("minutesAgo",i):s<24&&e.toDateString()===t.toDateString()?this._l10n.getString("hoursAgo",s):r<7?r===1?this._l10n.getString("yesterday"):this._l10n.getString("daysAgo",r):e.getFullYear()===t.getFullYear()?this._l10n.formatDate(e,{month:"short",day:"numeric"}):this._l10n.formatDate(e,{month:"short",day:"numeric",year:"numeric"})}async _getGitBlameInfo(e,t=null){if(!this._gitAvailable||!ae)return null;try{let o=m.workspace.getWorkspaceFolder(m.Uri.file(e));if(!o)return null;let i=o.uri.fsPath||o.uri.path,s=Jo(i,e),r=this._getGitCacheKey(i,s,t),n=this._getCachedGitInfo(r);if(n)return n;let l=Date.now();try{let{stdout:c}=await ae(`git log -1 --format="%H|%an|%ae|%ad" -- "${s}"`,{cwd:o.uri.fsPath,timeout:2e3});if(!c||!c.trim())return null;let[h,u,w,g]=c.trim().split("|"),v={hash:h||"",authorName:u||"Unknown",authorEmail:w||"",authorDate:g||""};return this._setCachedGitInfo(r,v),v}finally{let c=Date.now()-l;this._metrics.gitBlameTimeMs+=c,this._metrics.gitBlameCalls++}}catch{return null}}_getInitials(e){if(!e||typeof e!="string")return null;let t=e.trim().split(/\s+/).filter(Boolean);return t.length===0?null:t.length===1?t[0].substring(0,2).toUpperCase():(t[0][0]+(t[1][0]||"")).substring(0,2).toUpperCase()}_formatCompactSize(e){if(typeof e!="number"||isNaN(e))return null;let t=["B","K","M","G","T"],o=0,i=e;for(;i>=1024&&o<t.length-1;)i=i/1024,o++;let s=Math.round(i),r=t[o];if(s<=9)return`${s}${r}`;let n=String(s);return n.length>=2?n.slice(0,2):n}_formatFullDate(e){let t={year:"numeric",month:"long",day:"numeric",hour:"2-digit",minute:"2-digit",second:"2-digit",timeZoneName:"short"};return e.toLocaleString("en-US",t)}_getCacheKey(e){return Ko(Q(e))}_isFileNotFoundError(e){return e?e.code==="ENOENT"?!0:typeof e.message=="string"&&e.message.includes("ENOENT"):!1}async provideFileDecoration(e,t){var i,s,r,n,l;let o=Date.now();try{if(!e){this._logger.error("\u274C Invalid URI provided to provideFileDecoration:",e);return}let c=Q(e);if(!c){this._logger.error("\u274C Could not resolve path for URI in provideFileDecoration:",e);return}let h=G(c),u=e.scheme||"file";if(u!=="file"){this._logger.debug(`\u23ED\uFE0F Skipping decoration for ${h} (unsupported scheme: ${u})`);return}this._performanceMode||(this._logger.info(`\u{1F50D} VSCODE REQUESTED DECORATION: ${h} (${c})`),this._logger.info(`\u{1F4CA} Call context: token=${!!t}, cancelled=${t==null?void 0:t.isCancellationRequested}`));let w=m.workspace.getConfiguration("explorerDates"),g=d((F,dt)=>{if(this._previewSettings&&Object.prototype.hasOwnProperty.call(this._previewSettings,F)){let ht=this._previewSettings[F];return this._logger.debug(`\u{1F3AD} Using preview value for ${F}: ${ht} (config has: ${w.get(F,dt)})`),ht}return w.get(F,dt)},"_get");if(this._previewSettings&&this._logger.info(`\u{1F3AD} Processing ${h} in PREVIEW MODE with settings:`,this._previewSettings),!g("showDateDecorations",!0)){this._performanceMode||this._logger.info(`\u274C RETURNED UNDEFINED: Decorations disabled globally for ${h}`);return}if(await this._isExcludedSimple(e)){this._performanceMode||this._logger.info(`\u274C File excluded: ${h}`);return}this._logger.debug(`\u{1F50D} Processing file: ${h}`);let v=this._getCacheKey(e);if(this._previewSettings)this._logger.debug(`\u{1F504} Skipping cache due to active preview settings for: ${h}`);else{let F=await this._getCachedDecoration(v,h);if(F)return F}if(this._metrics.cacheMisses++,this._logger.debug(`\u274C Cache miss for: ${h} (key: ${v.substring(0,50)}...)`),t!=null&&t.isCancellationRequested){this._logger.debug(`Decoration cancelled for: ${c}`);return}let N=Date.now(),k;try{k=await this._fileSystem.stat(e)}catch(F){if(this._metrics.fileStatTimeMs+=Date.now()-N,this._metrics.fileStatCalls++,this._isFileNotFoundError(F)){this._logger.debug(`\u23ED\uFE0F Skipping decoration for ${h}: file not found (${F.message||F})`);return}throw F}if(this._metrics.fileStatTimeMs+=Date.now()-N,this._metrics.fileStatCalls++,!(typeof k.isFile=="function"?k.isFile():!0))return;let Z=k.mtime instanceof Date?k.mtime:new Date(k.mtime),be=k.birthtime instanceof Date?k.birthtime:new Date(k.birthtime||k.ctime||k.mtime),ce={mtime:Z,birthtime:be,size:k.size},B=Date.now()-Z.getTime(),le=g("dateDecorationFormat","smart"),H=this._performanceMode?"none":g("colorScheme","none"),ye=g("highContrastMode",!1),_e=this._performanceMode?!1:g("showFileSize",!1),y=g("fileSizeFormat","auto"),V=g("accessibilityMode",!1),Ce=this._performanceMode?!1:g("fadeOldFiles",!1),xe=g("fadeThreshold",30),te=this._performanceMode?"none":g("showGitInfo","none"),de=this._performanceMode?"time":g("badgePriority","time"),De=(te!=="none"||de==="author")&&this._gitAvailable&&!this._performanceMode,nt=De?te:"none";de==="author"&&!De&&(de="time");let ct=De?await this._getGitBlameInfo(c,Z.getTime()):null,Se=this._generateBadgeDetails({filePath:c,stat:ce,diffMs:B,dateFormat:le,badgePriority:de,showFileSize:_e,fileSizeFormat:y,gitBlame:ct,showGitInfo:nt}),so=me(c),ro=[".js",".ts",".jsx",".tsx",".py",".rb",".php",".java",".cpp",".c",".cs",".go",".rs",".kt",".swift"].includes(so),lt=V&&((i=this._accessibility)==null?void 0:i.shouldEnhanceAccessibility());this._logger.debug(`\u{1F50D} Tooltip generation for ${h}: accessibilityMode=${V}, shouldUseAccessible=${lt}, previewMode=${!!this._previewSettings}`);let he=await this._buildTooltipContent({filePath:c,resourceUri:e,stat:ce,badgeDetails:Se,gitBlame:nt==="none"?null:ct,shouldUseAccessibleTooltips:lt,fileSizeFormat:y,isCodeFile:ro}),q;H!=="none"&&(q=this._themeIntegration?this._themeIntegration.applyThemeAwareColorScheme(H,c,B):this._getColorByScheme(Z,H,c)),this._logger.debug(`\u{1F3A8} Color scheme setting: ${H}, using color: ${q?"yes":"no"}`),Ce&&Math.floor(B/864e5)>xe&&(q=new m.ThemeColor("editorGutter.commentRangeForeground"));let Fe=zt(Se.displayBadge)||zt(Se.badge)||"??";(s=this._accessibility)!=null&&s.shouldEnhanceAccessibility()&&(Fe=this._accessibility.getAccessibleBadge(Fe));let $;try{if($=new m.FileDecoration(Fe),he&&he.length<500&&($.tooltip=he,this._logger.debug(`\u{1F4DD} Added tooltip (${he.length} chars)`)),q){let F=this._enhanceColorForSelection(q);$.color=F,this._logger.debug(`\u{1F3A8} Added enhanced color: ${F.id||F} (original: ${q.id||q})`)}$.propagate=!1}catch(F){this._logger.error("\u274C Failed to create decoration:",F),$=new m.FileDecoration("!!"),$.propagate=!1}if(this._logger.debug(`\u{1F3A8} Color/contrast check for ${h}: colorScheme=${H}, highContrastMode=${ye}, hasColor=${!!q}, previewMode=${!!this._previewSettings}`),ye&&($.color=new m.ThemeColor("editorWarning.foreground"),this._logger.info(`\u{1F506} Applied high contrast color (overriding colorScheme=${H})`)),this._previewSettings?this._logger.debug(`\u{1F504} Skipping cache storage due to preview mode for: ${h}`):await this._storeDecorationInCache(v,$,h),this._metrics.totalDecorations++,!($!=null&&$.badge)){this._logger.error(`\u274C Decoration badge is invalid for: ${h}`);return}let ao=Date.now()-o;return this._performanceMode||(this._logger.info(`\u2705 Decoration created for: ${h} (badge: ${$.badge||"undefined"}) - Cache key: ${v.substring(0,30)}...`),this._logger.info("\u{1F3AF} RETURNING DECORATION TO VSCODE:",{file:h,badge:$.badge,hasTooltip:!!$.tooltip,hasColor:!!$.color,colorType:(n=(r=$.color)==null?void 0:r.constructor)==null?void 0:n.name,processingTimeMs:ao,decorationType:$.constructor.name})),$}catch(c){this._metrics.errors++;let h=o?Date.now()-o:0,u=G(e),w=Q(e)||"unknown-uri";this._logger.error(`\u274C DECORATION ERROR for ${u}:`,{error:c.message,stack:(l=c.stack)==null?void 0:l.split(`
`)[0],processingTimeMs:h,uri:w}),this._logger.error(`\u274C CRITICAL ERROR DETAILS for ${u}: ${c.message}`),this._logger.error(`\u274C Error type: ${c.constructor.name}`),this._logger.error(`\u274C Full stack: ${c.stack}`),this._logger.info(`\u274C RETURNED UNDEFINED: Error occurred for ${u}`);return}}getMetrics(){let e={...this._metrics,cacheSize:this._decorationCache.size,cacheHitRate:this._metrics.cacheHits+this._metrics.cacheMisses>0?(this._metrics.cacheHits/(this._metrics.cacheHits+this._metrics.cacheMisses)*100).toFixed(2)+"%":"0.00%"};return this._advancedCache&&(e.advancedCache=this._advancedCache.getStats()),this._batchProcessor&&(e.batchProcessor=this._batchProcessor.getMetrics()),e.cacheDebugging={memoryCacheKeys:Array.from(this._decorationCache.keys()).slice(0,5),cacheTimeout:this._cacheTimeout,maxCacheSize:this._maxCacheSize,keyStatsSize:this._cacheKeyStats?this._cacheKeyStats.size:0},e.performanceTiming={avgGitBlameMs:this._metrics.gitBlameCalls>0?(this._metrics.gitBlameTimeMs/this._metrics.gitBlameCalls).toFixed(1):"0.0",avgFileStatMs:this._metrics.fileStatCalls>0?(this._metrics.fileStatTimeMs/this._metrics.fileStatCalls).toFixed(1):"0.0",totalGitBlameTimeMs:this._metrics.gitBlameTimeMs,totalFileStatTimeMs:this._metrics.fileStatTimeMs,gitBlameCalls:this._metrics.gitBlameCalls,fileStatCalls:this._metrics.fileStatCalls},e}async initializeAdvancedSystems(e){try{if(this._extensionContext=e,this._isWeb&&await this._maybeWarnAboutGitLimitations(),this._performanceMode){this._logger.info("Performance mode enabled - skipping advanced cache, batch processor, and progressive loading");return}this._advancedCache=new Go(e),await this._advancedCache.initialize(),this._logger.info("Advanced cache initialized"),this._batchProcessor.initialize(),this._logger.info("Batch processor initialized"),await this._applyProgressiveLoadingSetting(),m.workspace.getConfiguration("explorerDates").get("autoThemeAdaptation",!0)&&(await this._themeIntegration.autoConfigureForTheme(),this._logger.info("Theme integration configured")),this._accessibility.shouldEnhanceAccessibility()&&(await this._accessibility.applyAccessibilityRecommendations(),this._logger.info("Accessibility recommendations applied"));try{await this._smartExclusion.cleanupAllWorkspaceProfiles()}catch(o){this._logger.warn("Failed to clean workspace exclusion profiles",o)}if(m.workspace.workspaceFolders)for(let o of m.workspace.workspaceFolders)try{await this._smartExclusion.suggestExclusions(o.uri),this._logger.info(`Smart exclusions analyzed for: ${o.name}`)}catch(i){this._logger.error(`Failed to analyze smart exclusions for ${o.name}`,i)}this._logger.info("Advanced systems initialized successfully")}catch(t){this._logger.error("Failed to initialize advanced systems",t)}}async _maybeWarnAboutGitLimitations(){var e;if(!this._gitWarningShown){this._gitWarningShown=!0;try{let t=(e=this._extensionContext)==null?void 0:e.globalState,o=Zo.WEB_GIT_NOTICE;if(t==null?void 0:t.get(o,!1))return;if(t!=null&&t.update)try{await t.update(o,!0)}catch(s){this._logger.debug("Failed to persist Git limitation notice flag",s)}Promise.resolve().then(()=>{m.window.showInformationMessage("Explorer Dates: Git attribution badges are unavailable on VS Code for Web. Time-based decorations remain available.")})}catch(t){this._logger.debug("Failed to display Git limitation notice",t)}}}_enhanceColorForSelection(e){let t={"charts.yellow":"list.warningForeground","charts.red":"list.errorForeground","charts.green":"list.highlightForeground","charts.blue":"symbolIcon.functionForeground","charts.purple":"symbolIcon.classForeground","charts.orange":"list.warningForeground","terminal.ansiYellow":"list.warningForeground","terminal.ansiGreen":"list.highlightForeground","terminal.ansiRed":"list.errorForeground","terminal.ansiBlue":"symbolIcon.functionForeground","terminal.ansiMagenta":"symbolIcon.classForeground","terminal.ansiCyan":"symbolIcon.stringForeground","editorGutter.commentRangeForeground":"list.deemphasizedForeground","editorWarning.foreground":"list.warningForeground","editorError.foreground":"list.errorForeground","editorInfo.foreground":"list.highlightForeground"},o=e.id||e,i=t[o];return i?(this._logger.debug(`\u{1F527} Enhanced color ${o} \u2192 ${i} for better selection visibility`),new m.ThemeColor(i)):e}async dispose(){this._logger.info("Disposing FileDateDecorationProvider",this.getMetrics()),this._refreshTimer&&(clearInterval(this._refreshTimer),this._refreshTimer=null,this._logger.info("Cleared periodic refresh timer")),this._advancedCache&&await this._advancedCache.dispose(),this._cancelProgressiveWarmupJobs(),this._batchProcessor&&this._batchProcessor.dispose(),this._accessibility&&typeof this._accessibility.dispose=="function"&&this._accessibility.dispose(),this._decorationCache.clear(),this._gitCache.clear(),this._onDidChangeFileDecorations.dispose(),this._fileWatcher&&this._fileWatcher.dispose(),this._configurationWatcher&&(this._configurationWatcher.dispose(),this._configurationWatcher=null)}};d(qe,"FileDateDecorationProvider");var He=qe;Lt.exports={FileDateDecorationProvider:He}});var Ot=S((exports,module)=>{var vscode=require("vscode"),{fileSystem}=j(),{getFileName,getRelativePath}=U(),isWeb=!1,childProcess=null;function loadChildProcess(){return!childProcess&&!isWeb&&(childProcess=eval("require")("child_process")),childProcess}d(loadChildProcess,"loadChildProcess");function registerCoreCommands({context:a,fileDateProvider:e,logger:t,l10n:o}){let i=[];i.push(vscode.commands.registerCommand("explorerDates.refreshDateDecorations",()=>{try{if(e){e.clearAllCaches(),e.refreshAll();let s=(o==null?void 0:o.getString("refreshSuccess"))||"Date decorations refreshed - all caches cleared";vscode.window.showInformationMessage(s),t.info("Date decorations refreshed manually with cache clear")}}catch(s){t.error("Failed to refresh decorations",s),vscode.window.showErrorMessage(`Failed to refresh decorations: ${s.message}`)}})),i.push(vscode.commands.registerCommand("explorerDates.previewConfiguration",s=>{try{e&&(e.applyPreviewSettings(s),t.info("Configuration preview applied",s))}catch(r){t.error("Failed to apply configuration preview",r)}})),i.push(vscode.commands.registerCommand("explorerDates.clearPreview",()=>{try{e&&(e.applyPreviewSettings(null),t.info("Configuration preview cleared"))}catch(s){t.error("Failed to clear configuration preview",s)}})),i.push(vscode.commands.registerCommand("explorerDates.showMetrics",()=>{try{if(e){let s=e.getMetrics(),r=`Explorer Dates Metrics:
...
explorerDatesApi.registerPlugin('myPlugin', myPlugin);
    ...
    </html>`}d(Fi,"getApiInformationHtml");function Ti(a){let e=d(o=>{if(o<1024)return`${o} B`;let i=o/1024;return i<1024?`${i.toFixed(1)} KB`:`${(i/1024).toFixed(1)} MB`},"formatFileSize"),t=a.map(o=>`
        <tr>
            <td>${o.path}</td>
    ...
    `}d(Ti,"generateWorkspaceActivityHTML");function ki(a){return`
        <!DOCTYPE html>
        <html>
        <head>
            <meta charset="UTF-8">
    ...
    `}d(ki,"generateDiagnosticsHTML");function $i(a){return`<!DOCTYPE html>
    <html>
    <head>
        <meta charset="UTF-8">
    ...
    </html>`}d($i,"generateDiagnosticsWebview");function Mi(a){let e=d(t=>{if(t===0)return"0 B";let o=1024,i=["B","KB","MB","GB"],s=Math.floor(Math.log(t)/Math.log(o));return parseFloat((t/Math.pow(o,s)).toFixed(2))+" "+i[s]},"formatBytes");return`
        <!DOCTYPE html>
        <html>
        <head>
            <meta charset="UTF-8">
    ...
    `}d(Mi,"generatePerformanceAnalyticsHTML");function io(a){let e=b.window.createStatusBarItem(b.StatusBarAlignment.Right,100);e.command="explorerDates.showFileDetails",e.tooltip="Click to show detailed file information";let t=d(async()=>{try{let o=b.window.activeTextEditor;if(!o){e.hide();return}let i=o.document.uri;if(i.scheme!=="file"){e.hide();return}let s=await Ci.stat(i),r=s.mtime instanceof Date?s.mtime:new Date(s.mtime),n=O._formatDateBadge(r,"smart"),l=O._formatFileSize(s.size,"auto");e.text=`$(clock) ${n} $(file) ${l}`,e.show()}catch(o){e.hide(),D.debug("Failed to update status bar",o)}},"updateStatusBar");return b.window.onDidChangeActiveTextEditor(t),b.window.onDidChangeTextEditorSelection(t),t(),a.subscriptions.push(e),e}d(io,"initializeStatusBar");async function Ei(a){try{D=yi(),ne=_i(),a.subscriptions.push(ne),D.info("Explorer Dates: Extension activated");let e=b.env.uiKind===b.UIKind.Web;await b.commands.executeCommand("setContext","explorerDates.gitFeaturesAvailable",!e);let t=b.workspace.getConfiguration("explorerDates"),o=t.get("enableWorkspaceTemplates",!0),i=t.get("enableReporting",!0),s=t.get("enableExtensionApi",!0);O=new bi;let r=b.window.registerFileDecorationProvider(O);a.subscriptions.push(r),a.subscriptions.push(O),a.subscriptions.push(D),await O.initializeAdvancedSystems(a);let n=null,l=null,c=null,h=null,u=d(()=>{if(!n){let{OnboardingManager:y}=Qt();n=new y(a)}return n},"getOnboardingManager"),w=d(()=>{if(!o)throw new Error("Workspace templates are disabled via explorerDates.enableWorkspaceTemplates");if(!l){let{WorkspaceTemplatesManager:y}=Zt();l=new y(a)}return l},"getWorkspaceTemplatesManager"),g=d(()=>{if(!c){let{ExtensionApiManager:y}=eo();c=new y,a.subscriptions.push(c)}return c},"getExtensionApiManager"),v=d(()=>{if(!i)throw new Error("Reporting is disabled via explorerDates.enableReporting");if(!h){let{ExportReportingManager:y}=oo();h=new y,a.subscriptions.push(h)}return h},"getExportReportingManager"),N=d(()=>g().getApi(),"apiFactory");s?a.exports=N:(a.exports=void 0,D.info("Explorer Dates API exports disabled via explorerDates.enableExtensionApi")),b.workspace.getConfiguration("explorerDates").get("showWelcomeOnStartup",!0)&&await u().shouldShowOnboarding()&&setTimeout(()=>{u().showWelcomeMessage()},5e3),xi({context:a,fileDateProvider:O,logger:D,l10n:ne}),Di({context:a,fileDateProvider:O,logger:D,generators:{generateWorkspaceActivityHTML:Ti,generatePerformanceAnalyticsHTML:Mi,generateDiagnosticsHTML:ki,generateDiagnosticsWebview:$i}}),Si({context:a,logger:D,getOnboardingManager:u});let at=b.commands.registerCommand("explorerDates.openTemplateManager",async()=>{try{if(!o){b.window.showInformationMessage("Workspace templates are disabled. Enable explorerDates.enableWorkspaceTemplates to use this command.");return}await w().showTemplateManager(),D.info("Template manager opened")}catch(y){D.error("Failed to open template manager",y),b.window.showErrorMessage(`Failed to open template manager: ${y.message}`)}});a.subscriptions.push(at);let Z=b.commands.registerCommand("explorerDates.saveTemplate",async()=>{try{if(!o){b.window.showInformationMessage("Workspace templates are disabled. Enable explorerDates.enableWorkspaceTemplates to save templates.");return}let y=await b.window.showInputBox({prompt:"Enter template name",placeHolder:"e.g., My Project Setup"});if(y){let V=await b.window.showInputBox({prompt:"Enter description (optional)",placeHolder:"Brief description of this template"})||"";await w().saveCurrentConfiguration(y,V)}D.info("Template saved")}catch(y){D.error("Failed to save template",y),b.window.showErrorMessage(`Failed to save template: ${y.message}`)}});a.subscriptions.push(Z);let be=b.commands.registerCommand("explorerDates.generateReport",async()=>{try{if(!i){b.window.showInformationMessage("Reporting features are disabled. Enable explorerDates.enableReporting to generate reports.");return}await v().showReportDialog(),D.info("Report generation started")}catch(y){D.error("Failed to generate report",y),b.window.showErrorMessage(`Failed to generate report: ${y.message}`)}});a.subscriptions.push(be);let ce=b.commands.registerCommand("explorerDates.showApiInfo",async()=>{try{if(!s){b.window.showInformationMessage("Explorer Dates API is disabled via settings.");return}let y=b.window.createWebviewPanel("apiInfo","Explorer Dates API Information",b.ViewColumn.One,{enableScripts:!0});y.webview.html=Fi(N()),D.info("API information panel opened")}catch(y){D.error("Failed to show API information",y),b.window.showErrorMessage(`Failed to show API information: ${y.message}`)}});a.subscriptions.push(ce);let B,le=b.workspace.getConfiguration("explorerDates"),H=le.get("performanceMode",!1);le.get("showStatusBar",!1)&&!H&&(B=io(a));let _e=b.workspace.onDidChangeConfiguration(y=>{if(y.affectsConfiguration("explorerDates.showStatusBar")||y.affectsConfiguration("explorerDates.performanceMode")){let V=b.workspace.getConfiguration("explorerDates"),Ce=V.get("showStatusBar",!1),xe=V.get("performanceMode",!1),te=Ce&&!xe;te&&!B?B=io(a):!te&&B&&(B.dispose(),B=null)}});a.subscriptions.push(_e),D.info("Explorer Dates: Date decorations ready")}catch(e){let t=`${ne?ne.getString("activationError"):"Explorer Dates failed to activate"}: ${e.message}`;throw D&&D.error("Extension activation failed",e),b.window.showErrorMessage(t),e}}d(Ei,"activate");async function Ai(){try{D&&D.info("Explorer Dates extension is being deactivated"),O&&typeof O.dispose=="function"&&await O.dispose(),D&&D.info("Explorer Dates extension deactivated successfully")}catch(a){D&&D.error("Explorer Dates: Error during deactivation",a)}}d(Ai,"deactivate");module.exports={activate:Ei,deactivate:Ai};
