...
\u{1F464} Last Modified By: ${gitBlame.authorName}`,gitBlame.authorEmail&&(tooltip+=` (${gitBlame.authorEmail})`),gitBlame.authorDate&&(tooltip+=`
   ${gitBlame.authorDate}`)),tooltip}_formatDateReadable(date){let now=new Date,diffMs=now.getTime()-date.getTime(),diffMins=Math.floor(diffMs/(1e3*60)),diffHours=Math.floor(diffMs/(1e3*60*60)),diffDays=Math.floor(diffMs/(1e3*60*60*24));return diffMins<1?this._l10n.getString("justNow"):diffMins<60?this._l10n.getString("minutesAgo",diffMins):diffHours<24&&date.toDateString()===now.toDateString()?this._l10n.getString("hoursAgo",diffHours):diffDays<7?diffDays===1?this._l10n.getString("yesterday"):this._l10n.getString("daysAgo",diffDays):date.getFullYear()===now.getFullYear()?this._l10n.formatDate(date,{month:"short",day:"numeric"}):this._l10n.formatDate(date,{month:"short",day:"numeric",year:"numeric"})}async _getGitBlameInfo(filePath){if(!this._gitAvailable||!execAsync)return null;try{let workspaceFolder=vscode3.workspace.getWorkspaceFolder(vscode3.Uri.file(filePath));if(!workspaceFolder)return null;let workspacePath=workspaceFolder.uri.fsPath||workspaceFolder.uri.path,relativePath=getRelativePath2(workspacePath,filePath),{stdout}=await execAsync(`git log -1 --format="%an|%ae|%ad" -- "${relativePath}"`,{cwd:workspaceFolder.uri.fsPath,timeout:2e3});if(!stdout||!stdout.trim())return null;let[authorName,authorEmail,authorDate]=stdout.trim().split("|");return{authorName:authorName||"Unknown",authorEmail:authorEmail||"",authorDate:authorDate||""}}catch{return null}}_getInitials(fullName){if(!fullName||typeof fullName!="string")return null;let parts=fullName.trim().split(/\s+/).filter(Boolean);return parts.length===0?null:parts.length===1?parts[0].substring(0,2).toUpperCase():(parts[0][0]+(parts[1][0]||"")).substring(0,2).toUpperCase()}_formatCompactSize(bytes){if(typeof bytes!="number"||isNaN(bytes))return null;let units=["B","K","M","G","T"],i=0,val=bytes;for(;val>=1024&&i<units.length-1;)val=val/1024,i++;let rounded=Math.round(val),unit=units[i];if(rounded<=9)return`${rounded}${unit}`;let s=String(rounded);return s.length>=2?s.slice(0,2):s}_formatFullDate(date){let options={year:"numeric",month:"long",day:"numeric",hour:"2-digit",minute:"2-digit",second:"2-digit",timeZoneName:"short"};return date.toLocaleString("en-US",options)}_getCacheKey(uri){return buildCacheKey(getUriPath(uri))}async provideFileDecoration(uri,token){var _a,_b,_c,_d,_e;let startTime=Date.now();try{if(!uri){this._logger.error("\u274C Invalid URI provided to provideFileDecoration:",uri);return}let filePath=getUriPath(uri);if(!filePath){this._logger.error("\u274C Could not resolve path for URI in provideFileDecoration:",uri);return}let fileLabel=describeFile(filePath);this._performanceMode||(this._logger.info(`\u{1F50D} VSCODE REQUESTED DECORATION: ${fileLabel} (${filePath})`),this._logger.info(`\u{1F4CA} Call context: token=${!!token}, cancelled=${token==null?void 0:token.isCancellationRequested}`));let config=vscode3.workspace.getConfiguration("explorerDates"),_get=__name((key,def)=>{if(this._previewSettings&&Object.prototype.hasOwnProperty.call(this._previewSettings,key)){let previewValue=this._previewSettings[key];return this._logger.debug(`\u{1F3AD} Using preview value for ${key}: ${previewValue} (config has: ${config.get(key,def)})`),previewValue}return config.get(key,def)},"_get");if(this._previewSettings&&this._logger.info(`\u{1F3AD} Processing ${fileLabel} in PREVIEW MODE with settings:`,this._previewSettings),!_get("showDateDecorations",!0)){this._performanceMode||this._logger.info(`\u274C RETURNED UNDEFINED: Decorations disabled globally for ${fileLabel}`);return}if(await this._isExcludedSimple(uri)){this._performanceMode||this._logger.info(`\u274C File excluded: ${fileLabel}`);return}this._logger.debug(`\u{1F50D} Processing file: ${fileLabel}`);let cacheKey=this._getCacheKey(uri);if(this._previewSettings)this._logger.debug(`\u{1F504} Skipping cache due to active preview settings for: ${fileLabel}`);else{let cachedDecoration=await this._getCachedDecoration(cacheKey,fileLabel);if(cachedDecoration)return cachedDecoration}if(this._metrics.cacheMisses++,this._logger.debug(`\u274C Cache miss for: ${fileLabel} (key: ${cacheKey.substring(0,50)}...)`),token!=null&&token.isCancellationRequested){this._logger.debug(`Decoration cancelled for: ${filePath}`);return}let stat=await this._fileSystem.stat(uri);if(!(typeof stat.isFile=="function"?stat.isFile():!0))return;let modifiedAt=stat.mtime instanceof Date?stat.mtime:new Date(stat.mtime),createdAt=stat.birthtime instanceof Date?stat.birthtime:new Date(stat.birthtime||stat.ctime||stat.mtime),normalizedStat={mtime:modifiedAt,birthtime:createdAt,size:stat.size},diffMs=Date.now()-modifiedAt.getTime(),dateFormat=_get("dateDecorationFormat","smart"),colorScheme=this._performanceMode?"none":_get("colorScheme","none"),highContrastMode=_get("highContrastMode",!1),showFileSize=this._performanceMode?!1:_get("showFileSize",!1),fileSizeFormat=_get("fileSizeFormat","auto"),accessibilityMode=_get("accessibilityMode",!1),fadeOldFiles=this._performanceMode?!1:_get("fadeOldFiles",!1),fadeThreshold=_get("fadeThreshold",30),rawShowGitInfo=this._performanceMode?"none":_get("showGitInfo","none"),badgePriority=this._performanceMode?"time":_get("badgePriority","time"),gitFeaturesEnabled=(rawShowGitInfo!=="none"||badgePriority==="author")&&this._gitAvailable&&!this._performanceMode,showGitInfo=gitFeaturesEnabled?rawShowGitInfo:"none";badgePriority==="author"&&!gitFeaturesEnabled&&(badgePriority="time");let gitBlame=gitFeaturesEnabled?await this._getGitBlameInfo(filePath):null,badgeDetails=this._generateBadgeDetails({filePath,stat:normalizedStat,diffMs,dateFormat,badgePriority,showFileSize,fileSizeFormat,gitBlame,showGitInfo}),fileExt=getExtension(filePath),isCodeFile=[".js",".ts",".jsx",".tsx",".py",".rb",".php",".java",".cpp",".c",".cs",".go",".rs",".kt",".swift"].includes(fileExt),shouldUseAccessibleTooltips=accessibilityMode&&((_a=this._accessibility)==null?void 0:_a.shouldEnhanceAccessibility());this._logger.debug(`\u{1F50D} Tooltip generation for ${fileLabel}: accessibilityMode=${accessibilityMode}, shouldUseAccessible=${shouldUseAccessibleTooltips}, previewMode=${!!this._previewSettings}`);let tooltip=await this._buildTooltipContent({filePath,resourceUri:uri,stat:normalizedStat,badgeDetails,gitBlame:showGitInfo==="none"?null:gitBlame,shouldUseAccessibleTooltips,fileSizeFormat,isCodeFile}),color;colorScheme!=="none"&&(color=this._themeIntegration?this._themeIntegration.applyThemeAwareColorScheme(colorScheme,filePath,diffMs):this._getColorByScheme(modifiedAt,colorScheme,filePath)),this._logger.debug(`\u{1F3A8} Color scheme setting: ${colorScheme}, using color: ${color?"yes":"no"}`),fadeOldFiles&&Math.floor(diffMs/864e5)>fadeThreshold&&(color=new vscode3.ThemeColor("editorGutter.commentRangeForeground"));let finalBadge=trimBadge(badgeDetails.displayBadge)||trimBadge(badgeDetails.badge)||"??";(_b=this._accessibility)!=null&&_b.shouldEnhanceAccessibility()&&(finalBadge=this._accessibility.getAccessibleBadge(finalBadge));let decoration;try{if(decoration=new vscode3.FileDecoration(finalBadge),tooltip&&tooltip.length<500&&(decoration.tooltip=tooltip,this._logger.debug(`\u{1F4DD} Added tooltip (${tooltip.length} chars)`)),color){let enhancedColor=this._enhanceColorForSelection(color);decoration.color=enhancedColor,this._logger.debug(`\u{1F3A8} Added enhanced color: ${enhancedColor.id||enhancedColor} (original: ${color.id||color})`)}decoration.propagate=!1}catch(decorationError){this._logger.error("\u274C Failed to create decoration:",decorationError),decoration=new vscode3.FileDecoration("!!"),decoration.propagate=!1}if(this._logger.debug(`\u{1F3A8} Color/contrast check for ${fileLabel}: colorScheme=${colorScheme}, highContrastMode=${highContrastMode}, hasColor=${!!color}, previewMode=${!!this._previewSettings}`),highContrastMode&&(decoration.color=new vscode3.ThemeColor("editorWarning.foreground"),this._logger.info(`\u{1F506} Applied high contrast color (overriding colorScheme=${colorScheme})`)),this._previewSettings?this._logger.debug(`\u{1F504} Skipping cache storage due to preview mode for: ${fileLabel}`):await this._storeDecorationInCache(cacheKey,decoration,fileLabel),this._metrics.totalDecorations++,!(decoration!=null&&decoration.badge)){this._logger.error(`\u274C Decoration badge is invalid for: ${fileLabel}`);return}let processingTime=Date.now()-startTime;return this._performanceMode||(this._logger.info(`\u2705 Decoration created for: ${fileLabel} (badge: ${decoration.badge||"undefined"}) - Cache key: ${cacheKey.substring(0,30)}...`),this._logger.info("\u{1F3AF} RETURNING DECORATION TO VSCODE:",{file:fileLabel,badge:decoration.badge,hasTooltip:!!decoration.tooltip,hasColor:!!decoration.color,colorType:(_d=(_c=decoration.color)==null?void 0:_c.constructor)==null?void 0:_d.name,processingTimeMs:processingTime,decorationType:decoration.constructor.name}),console.log(`\u{1F3AF} DECORATION RETURNED: ${fileLabel} \u2192 "${decoration.badge}"`)),decoration}catch(error){this._metrics.errors++;let processingTime=startTime?Date.now()-startTime:0,safeFileName=describeFile(uri),safeUri=getUriPath(uri)||"unknown-uri";this._logger.error(`\u274C DECORATION ERROR for ${safeFileName}:`,{error:error.message,stack:(_e=error.stack)==null?void 0:_e.split(`
`)[0],processingTimeMs:processingTime,uri:safeUri}),console.error(`\u274C DECORATION ERROR: ${safeFileName} \u2192 ${error.message}`),console.error("\u274C Full error:",error),console.error("\u274C Stack trace:",error.stack),this._logger.error(`\u274C CRITICAL ERROR DETAILS for ${safeFileName}: ${error.message}`),this._logger.error(`\u274C Error type: ${error.constructor.name}`),this._logger.error(`\u274C Full stack: ${error.stack}`),this._logger.info(`\u274C RETURNED UNDEFINED: Error occurred for ${safeFileName}`);return}}getMetrics(){let baseMetrics={...this._metrics,cacheSize:this._decorationCache.size,cacheHitRate:this._metrics.cacheHits+this._metrics.cacheMisses>0?(this._metrics.cacheHits/(this._metrics.cacheHits+this._metrics.cacheMisses)*100).toFixed(2)+"%":"0.00%"};return this._advancedCache&&(baseMetrics.advancedCache=this._advancedCache.getStats()),this._batchProcessor&&(baseMetrics.batchProcessor=this._batchProcessor.getMetrics()),baseMetrics.cacheDebugging={memoryCacheKeys:Array.from(this._decorationCache.keys()).slice(0,5),cacheTimeout:this._cacheTimeout,maxCacheSize:this._maxCacheSize,keyStatsSize:this._cacheKeyStats?this._cacheKeyStats.size:0},baseMetrics}async initializeAdvancedSystems(context){try{if(this._extensionContext=context,this._isWeb&&await this._maybeWarnAboutGitLimitations(),this._performanceMode){this._logger.info("Performance mode enabled - skipping advanced cache, batch processor, and progressive loading");return}this._advancedCache=new AdvancedCache(context),await this._advancedCache.initialize(),this._logger.info("Advanced cache initialized"),this._batchProcessor.initialize(),this._logger.info("Batch processor initialized"),await this._applyProgressiveLoadingSetting(),vscode3.workspace.getConfiguration("explorerDates").get("autoThemeAdaptation",!0)&&(await this._themeIntegration.autoConfigureForTheme(),this._logger.info("Theme integration configured")),this._accessibility.shouldEnhanceAccessibility()&&(await this._accessibility.applyAccessibilityRecommendations(),this._logger.info("Accessibility recommendations applied"));try{await this._smartExclusion.cleanupAllWorkspaceProfiles()}catch(error){this._logger.warn("Failed to clean workspace exclusion profiles",error)}if(vscode3.workspace.workspaceFolders)for(let folder of vscode3.workspace.workspaceFolders)try{await this._smartExclusion.suggestExclusions(folder.uri),this._logger.info(`Smart exclusions analyzed for: ${folder.name}`)}catch(error){this._logger.error(`Failed to analyze smart exclusions for ${folder.name}`,error)}this._logger.info("Advanced systems initialized successfully")}catch(error){this._logger.error("Failed to initialize advanced systems",error)}}async _maybeWarnAboutGitLimitations(){var _a;if(!this._gitWarningShown){this._gitWarningShown=!0;try{let storage=(_a=this._extensionContext)==null?void 0:_a.globalState,storageKey=GLOBAL_STATE_KEYS.WEB_GIT_NOTICE;if(storage==null?void 0:storage.get(storageKey,!1))return;if(storage!=null&&storage.update)try{await storage.update(storageKey,!0)}catch(storageError){this._logger.debug("Failed to persist Git limitation notice flag",storageError)}Promise.resolve().then(()=>{vscode3.window.showInformationMessage("Explorer Dates: Git attribution badges are unavailable on VS Code for Web. Time-based decorations remain available.")})}catch(error){this._logger.debug("Failed to display Git limitation notice",error)}}}_enhanceColorForSelection(color){let colorEnhancementMap={"charts.yellow":"list.warningForeground","charts.red":"list.errorForeground","charts.green":"list.highlightForeground","charts.blue":"symbolIcon.functionForeground","charts.purple":"symbolIcon.classForeground","charts.orange":"list.warningForeground","terminal.ansiYellow":"list.warningForeground","terminal.ansiGreen":"list.highlightForeground","terminal.ansiRed":"list.errorForeground","terminal.ansiBlue":"symbolIcon.functionForeground","terminal.ansiMagenta":"symbolIcon.classForeground","terminal.ansiCyan":"symbolIcon.stringForeground","editorGutter.commentRangeForeground":"list.deemphasizedForeground","editorWarning.foreground":"list.warningForeground","editorError.foreground":"list.errorForeground","editorInfo.foreground":"list.highlightForeground"},colorId=color.id||color,enhancedColorId=colorEnhancementMap[colorId];return enhancedColorId?(this._logger.debug(`\u{1F527} Enhanced color ${colorId} \u2192 ${enhancedColorId} for better selection visibility`),new vscode3.ThemeColor(enhancedColorId)):color}async dispose(){this._logger.info("Disposing FileDateDecorationProvider",this.getMetrics()),this._advancedCache&&await this._advancedCache.dispose(),this._cancelProgressiveWarmupJobs(),this._batchProcessor&&this._batchProcessor.dispose(),this._decorationCache.clear(),this._onDidChangeFileDecorations.dispose(),this._fileWatcher&&this._fileWatcher.dispose()}};__name(_FileDateDecorationProvider,"FileDateDecorationProvider");var FileDateDecorationProvider2=_FileDateDecorationProvider;module2.exports={FileDateDecorationProvider:FileDateDecorationProvider2}}});var require_coreCommands=__commonJS({"src/commands/coreCommands.js"(exports,module){var vscode=require("vscode"),{fileSystem}=require_FileSystemAdapter(),{getFileName,getRelativePath}=require_pathUtils(),isWeb=!1,childProcess=null;function loadChildProcess(){return!childProcess&&!isWeb&&(childProcess=eval("require")("child_process")),childProcess}__name(loadChildProcess,"loadChildProcess");function registerCoreCommands({context,fileDateProvider:fileDateProvider2,logger:logger2,l10n:l10n2}){let subscriptions=[];subscriptions.push(vscode.commands.registerCommand("explorerDates.refreshDateDecorations",()=>{try{if(fileDateProvider2){fileDateProvider2.clearAllCaches(),fileDateProvider2.refreshAll();let message=(l10n2==null?void 0:l10n2.getString("refreshSuccess"))||"Date decorations refreshed - all caches cleared";vscode.window.showInformationMessage(message),logger2.info("Date decorations refreshed manually with cache clear")}}catch(error){logger2.error("Failed to refresh decorations",error),vscode.window.showErrorMessage(`Failed to refresh decorations: ${error.message}`)}})),subscriptions.push(vscode.commands.registerCommand("explorerDates.previewConfiguration",settings=>{try{fileDateProvider2&&(fileDateProvider2.applyPreviewSettings(settings),logger2.info("Configuration preview applied",settings))}catch(error){logger2.error("Failed to apply configuration preview",error)}})),subscriptions.push(vscode.commands.registerCommand("explorerDates.clearPreview",()=>{try{fileDateProvider2&&(fileDateProvider2.applyPreviewSettings(null),logger2.info("Configuration preview cleared"))}catch(error){logger2.error("Failed to clear configuration preview",error)}})),subscriptions.push(vscode.commands.registerCommand("explorerDates.showMetrics",()=>{try{if(fileDateProvider2){let metrics=fileDateProvider2.getMetrics(),message=`Explorer Dates Metrics:
...
Average Batch Time: ${metrics.batchProcessor.averageBatchTime.toFixed(2)}ms`),vscode.window.showInformationMessage(message,{modal:!0}),logger2.info("Metrics displayed",metrics)}}catch(error){logger2.error("Failed to show metrics",error),vscode.window.showErrorMessage(`Failed to show metrics: ${error.message}`)}})),subscriptions.push(vscode.commands.registerCommand("explorerDates.openLogs",()=>{try{logger2.show()}catch(error){logger2.error("Failed to open logs",error),vscode.window.showErrorMessage(`Failed to open logs: ${error.message}`)}})),subscriptions.push(vscode.commands.registerCommand("explorerDates.showCurrentConfig",()=>{try{let config=vscode.workspace.getConfiguration("explorerDates"),settings={highContrastMode:config.get("highContrastMode"),badgePriority:config.get("badgePriority"),colorScheme:config.get("colorScheme"),accessibilityMode:config.get("accessibilityMode"),dateDecorationFormat:config.get("dateDecorationFormat"),showGitInfo:config.get("showGitInfo"),showFileSize:config.get("showFileSize")},message=`Current Explorer Dates Configuration:

${Object.entries(settings).map(([key,value])=>`${key}: ${JSON.stringify(value)}`).join(`
`)}`;vscode.window.showInformationMessage(message,{modal:!0}),logger2.info("Current configuration displayed",settings)}catch(error){logger2.error("Failed to show configuration",error)}})),subscriptions.push(vscode.commands.registerCommand("explorerDates.resetToDefaults",async()=>{try{let config=vscode.workspace.getConfiguration("explorerDates");await config.update("highContrastMode",!1,vscode.ConfigurationTarget.Global),await config.update("badgePriority","time",vscode.ConfigurationTarget.Global),await config.update("accessibilityMode",!1,vscode.ConfigurationTarget.Global),vscode.window.showInformationMessage("Reset high contrast, badge priority, and accessibility mode to defaults. Changes should take effect immediately."),logger2.info("Reset problematic settings to defaults"),fileDateProvider2&&(fileDateProvider2.clearAllCaches(),fileDateProvider2.refreshAll())}catch(error){logger2.error("Failed to reset settings",error),vscode.window.showErrorMessage(`Failed to reset settings: ${error.message}`)}})),subscriptions.push(vscode.commands.registerCommand("explorerDates.toggleDecorations",()=>{try{let config=vscode.workspace.getConfiguration("explorerDates"),currentValue=config.get("showDateDecorations",!0);config.update("showDateDecorations",!currentValue,vscode.ConfigurationTarget.Global);let message=currentValue?(l10n2==null?void 0:l10n2.getString("decorationsDisabled"))||"Date decorations disabled":(l10n2==null?void 0:l10n2.getString("decorationsEnabled"))||"Date decorations enabled";vscode.window.showInformationMessage(message),logger2.info(`Date decorations toggled to: ${!currentValue}`)}catch(error){logger2.error("Failed to toggle decorations",error),vscode.window.showErrorMessage(`Failed to toggle decorations: ${error.message}`)}})),subscriptions.push(vscode.commands.registerCommand("explorerDates.copyFileDate",async uri=>{try{let targetUri=uri;if(!targetUri&&vscode.window.activeTextEditor&&(targetUri=vscode.window.activeTextEditor.document.uri),!targetUri){vscode.window.showWarningMessage("No file selected");return}let stat=await fileSystem.stat(targetUri),dateString=(stat.mtime instanceof Date?stat.mtime:new Date(stat.mtime)).toLocaleString();await vscode.env.clipboard.writeText(dateString),vscode.window.showInformationMessage(`Copied to clipboard: ${dateString}`),logger2.info(`File date copied for: ${targetUri.fsPath||targetUri.path}`)}catch(error){logger2.error("Failed to copy file date",error),vscode.window.showErrorMessage(`Failed to copy file date: ${error.message}`)}})),subscriptions.push(vscode.commands.registerCommand("explorerDates.showFileDetails",async uri=>{try{let targetUri=uri;if(!targetUri&&vscode.window.activeTextEditor&&(targetUri=vscode.window.activeTextEditor.document.uri),!targetUri){vscode.window.showWarningMessage("No file selected");return}let stat=await fileSystem.stat(targetUri),fileName=getFileName(targetUri.fsPath||targetUri.path),fileSize=(fileDateProvider2==null?void 0:fileDateProvider2._formatFileSize(stat.size,"auto"))||`${stat.size} bytes`,modified=(stat.mtime instanceof Date?stat.mtime:new Date(stat.mtime)).toLocaleString(),created=(stat.birthtime instanceof Date?stat.birthtime:new Date(stat.birthtime||stat.mtime)).toLocaleString(),details=`File: ${fileName}
Size: ${fileSize}
Modified: ${modified}
Created: ${created}
Path: ${targetUri.fsPath||targetUri.path}`;vscode.window.showInformationMessage(details,{modal:!0}),logger2.info(`File details shown for: ${targetUri.fsPath||targetUri.path}`)}catch(error){logger2.error("Failed to show file details",error),vscode.window.showErrorMessage(`Failed to show file details: ${error.message}`)}})),subscriptions.push(vscode.commands.registerCommand("explorerDates.toggleFadeOldFiles",()=>{try{let config=vscode.workspace.getConfiguration("explorerDates"),currentValue=config.get("fadeOldFiles",!1);config.update("fadeOldFiles",!currentValue,vscode.ConfigurationTarget.Global);let message=currentValue?"Fade old files disabled":"Fade old files enabled";vscode.window.showInformationMessage(message),logger2.info(`Fade old files toggled to: ${!currentValue}`)}catch(error){logger2.error("Failed to toggle fade old files",error),vscode.window.showErrorMessage(`Failed to toggle fade old files: ${error.message}`)}})),subscriptions.push(vscode.commands.registerCommand("explorerDates.showFileHistory",async uri=>{try{if(isWeb){vscode.window.showInformationMessage("Git history is unavailable on VS Code for Web.");return}let targetUri=uri;if(!targetUri&&vscode.window.activeTextEditor&&(targetUri=vscode.window.activeTextEditor.document.uri),!targetUri){vscode.window.showWarningMessage("No file selected");return}let workspaceFolder=vscode.workspace.getWorkspaceFolder(targetUri);if(!workspaceFolder){vscode.window.showWarningMessage("File is not in a workspace");return}let command=`git log --oneline -10 -- "${getRelativePath(workspaceFolder.uri.fsPath||workspaceFolder.uri.path,targetUri.fsPath||targetUri.path)}"`;loadChildProcess().exec(command,{cwd:workspaceFolder.uri.fsPath,timeout:3e3},(error,stdout)=>{if(error){error.message.includes("not a git repository")?vscode.window.showWarningMessage("This file is not in a Git repository"):vscode.window.showErrorMessage(`Git error: ${error.message}`);return}if(!stdout.trim()){vscode.window.showInformationMessage("No Git history found for this file");return}let history=stdout.trim(),fileName=getFileName(targetUri.fsPath||targetUri.path);vscode.window.showInformationMessage(`Recent commits for ${fileName}:

${history}`,{modal:!0})}),logger2.info(`File history requested for: ${targetUri.fsPath||targetUri.path}`)}catch(error){logger2.error("Failed to show file history",error),vscode.window.showErrorMessage(`Failed to show file history: ${error.message}`)}})),subscriptions.push(vscode.commands.registerCommand("explorerDates.compareWithPrevious",async uri=>{try{if(isWeb){vscode.window.showInformationMessage("Git comparisons are unavailable on VS Code for Web.");return}let targetUri=uri;if(!targetUri&&vscode.window.activeTextEditor&&(targetUri=vscode.window.activeTextEditor.document.uri),!targetUri){vscode.window.showWarningMessage("No file selected");return}if(!vscode.workspace.getWorkspaceFolder(targetUri)){vscode.window.showWarningMessage("File is not in a workspace");return}await vscode.commands.executeCommand("git.openChange",targetUri),logger2.info(`Git diff opened for: ${targetUri.fsPath||targetUri.path}`)}catch(error){logger2.error("Failed to compare with previous version",error),vscode.window.showErrorMessage(`Failed to compare with previous version: ${error.message}`)}})),subscriptions.forEach(disposable=>context.subscriptions.push(disposable))}__name(registerCoreCommands,"registerCoreCommands");module.exports={registerCoreCommands}}});var require_decorationDiagnostics=__commonJS({"src/decorationDiagnostics.js"(exports2,module2){var vscode3=require("vscode"),{getLogger:getLogger2}=require_logger(),_DecorationDiagnostics=class _DecorationDiagnostics{constructor(decorationProvider){this._logger=getLogger2(),this._provider=decorationProvider,this._testResults=[]}async runComprehensiveDiagnostics(){var _a,_b;this._logger.info("\u{1F50D} Starting comprehensive decoration diagnostics...");let results={timestamp:new Date().toISOString(),vscodeVersion:vscode3.version,extensionVersion:(_b=(_a=vscode3.extensions.getExtension("incredincomp.explorer-dates"))==null?void 0:_a.packageJSON)==null?void 0:_b.version,tests:{}};return results.tests.vscodeSettings=await this._testVSCodeSettings(),results.tests.providerRegistration=await this._testProviderRegistration(),results.tests.fileProcessing=await this._testFileProcessing(),results.tests.decorationCreation=await this._testDecorationCreation(),results.tests.cacheAnalysis=await this._testCacheAnalysis(),results.tests.extensionConflicts=await this._testExtensionConflicts(),results.tests.uriPathIssues=await this._testURIPathIssues(),this._logger.info("\u{1F50D} Comprehensive diagnostics completed",results),results}async _testVSCodeSettings(){let explorerConfig=vscode3.workspace.getConfiguration("explorer"),workbenchConfig=vscode3.workspace.getConfiguration("workbench"),explorerDatesConfig=vscode3.workspace.getConfiguration("explorerDates"),settings={"explorer.decorations.badges":explorerConfig.get("decorations.badges"),"explorer.decorations.colors":explorerConfig.get("decorations.colors"),"workbench.colorTheme":workbenchConfig.get("colorTheme"),"explorerDates.showDateDecorations":explorerDatesConfig.get("showDateDecorations"),"explorerDates.colorScheme":explorerDatesConfig.get("colorScheme"),"explorerDates.showGitInfo":explorerDatesConfig.get("showGitInfo")},issues=[];return settings["explorer.decorations.badges"]===!1&&issues.push("CRITICAL: explorer.decorations.badges is disabled"),settings["explorer.decorations.colors"]===!1&&issues.push("WARNING: explorer.decorations.colors is disabled"),settings["explorerDates.showDateDecorations"]===!1&&issues.push("CRITICAL: explorerDates.showDateDecorations is disabled"),{status:issues.length>0?"ISSUES_FOUND":"OK",settings,issues}}async _testProviderRegistration(){let issues=[];if(!this._provider)return issues.push("CRITICAL: Decoration provider is null/undefined"),{status:"FAILED",issues};typeof this._provider.provideFileDecoration!="function"&&issues.push("CRITICAL: provideFileDecoration method missing"),this._provider.onDidChangeFileDecorations||issues.push("WARNING: onDidChangeFileDecorations event emitter missing");let testUri=vscode3.Uri.file("/test/path");try{let testResult=await this._provider.provideFileDecoration(testUri);this._logger.debug("Provider test call completed",{result:!!testResult})}catch(error){issues.push(`ERROR: Provider test call failed: ${error.message}`)}return{status:issues.length>0?"ISSUES_FOUND":"OK",providerActive:!!this._provider,issues}}async _testFileProcessing(){let workspaceFolders=vscode3.workspace.workspaceFolders;if(!workspaceFolders||workspaceFolders.length===0)return{status:"NO_WORKSPACE",issues:["No workspace folders available"]};let testFiles=[],issues=[];try{let commonFiles=["package.json","README.md","extension.js","src/logger.js"];for(let fileName of commonFiles){let fileUri=vscode3.Uri.joinPath(workspaceFolders[0].uri,fileName);try{await vscode3.workspace.fs.stat(fileUri);let isExcluded=this._provider._isExcludedSimple?await this._provider._isExcludedSimple(fileUri):!1,decoration=await this._provider.provideFileDecoration(fileUri);testFiles.push({file:fileName,exists:!0,excluded:isExcluded,hasDecoration:!!decoration,badge:decoration==null?void 0:decoration.badge,uri:fileUri.toString()})}catch(fileError){testFiles.push({file:fileName,exists:!1,error:fileError.message})}}}catch(error){issues.push(`File processing test failed: ${error.message}`)}return{status:issues.length>0?"ISSUES_FOUND":"OK",testFiles,issues}}async _testDecorationCreation(){let tests=[],issues=[];try{let simpleDecoration=new vscode3.FileDecoration("test");tests.push({name:"Simple decoration",success:!0,badge:simpleDecoration.badge})}catch(error){tests.push({name:"Simple decoration",success:!1,error:error.message}),issues.push("CRITICAL: Cannot create simple FileDecoration")}try{let tooltipDecoration=new vscode3.FileDecoration("test","Test tooltip");tests.push({name:"Decoration with tooltip",success:!0,hasTooltip:!!(tooltipDecoration&&tooltipDecoration.tooltip)})}catch(error){tests.push({name:"Decoration with tooltip",success:!1,error:error.message}),issues.push("WARNING: Cannot create FileDecoration with tooltip")}try{let colorDecoration=new vscode3.FileDecoration("test","Test tooltip",new vscode3.ThemeColor("charts.red"));tests.push({name:"Decoration with color",success:!0,hasColor:!!colorDecoration.color})}catch(error){tests.push({name:"Decoration with color",success:!1,error:error.message}),issues.push("WARNING: Cannot create FileDecoration with color")}let badgeTests=["1d","10m","2h","!!","\u25CF\u25CF","JA12","123456789"];for(let badge of badgeTests)try{let badgeDecoration=new vscode3.FileDecoration(badge);tests.push({name:`Badge format: ${badge}`,success:!0,badge:badgeDecoration.badge,length:badge.length})}catch(error){tests.push({name:`Badge format: ${badge}`,success:!1,error:error.message}),badge.length<=8&&issues.push(`WARNING: Valid badge format '${badge}' failed`)}return{status:issues.length>0?"ISSUES_FOUND":"OK",tests,issues}}async _testCacheAnalysis(){var _a;let cacheInfo={memoryCache:{size:((_a=this._provider._decorationCache)==null?void 0:_a.size)||0,maxSize:this._provider._maxCacheSize||0},advancedCache:{available:!!this._provider._advancedCache,initialized:!1},metrics:this._provider.getMetrics?this._provider.getMetrics():null},issues=[];return cacheInfo.memoryCache.size>cacheInfo.memoryCache.maxSize*.9&&issues.push("WARNING: Memory cache is nearly full"),cacheInfo.metrics&&cacheInfo.metrics.cacheHits===0&&cacheInfo.metrics.cacheMisses>10&&issues.push("WARNING: Cache hit rate is 0% - potential cache key issues"),{status:issues.length>0?"ISSUES_FOUND":"OK",cacheInfo,issues}}async _testExtensionConflicts(){var _a,_b;let allExtensions=vscode3.extensions.all,potentialConflicts=[],decorationExtensions=[];for(let ext of allExtensions){if(!ext.isActive)continue;let packageJson=ext.packageJSON;(_b=(_a=packageJson.contributes)==null?void 0:_a.commands)!=null&&_b.some(cmd=>{var _a2,_b2,_c,_d;return((_a2=cmd.command)==null?void 0:_a2.includes("decoration"))||((_b2=cmd.title)==null?void 0:_b2.includes("decoration"))||((_c=cmd.title)==null?void 0:_c.includes("badge"))||((_d=cmd.title)==null?void 0:_d.includes("explorer"))})&&decorationExtensions.push({id:ext.id,name:packageJson.displayName||packageJson.name,version:packageJson.version}),["file-icons","vscode-icons","material-icon-theme","explorer-exclude","hide-files","file-watcher"].some(conflict=>ext.id.includes(conflict))&&potentialConflicts.push({id:ext.id,name:packageJson.displayName||packageJson.name,reason:"Known to potentially interfere with file decorations"})}let issues=[];return decorationExtensions.length>1&&issues.push(`WARNING: ${decorationExtensions.length} extensions might provide file decorations`),potentialConflicts.length>0&&issues.push(`WARNING: ${potentialConflicts.length} potentially conflicting extensions detected`),{status:issues.length>0?"ISSUES_FOUND":"OK",decorationExtensions,potentialConflicts,issues}}async _testURIPathIssues(){let workspaceFolders=vscode3.workspace.workspaceFolders;if(!workspaceFolders||workspaceFolders.length===0)return{status:"NO_WORKSPACE",issues:["No workspace available for URI testing"]};let tests=[],issues=[],testPaths=["package.json","src/logger.js","README.md",".gitignore"];for(let testPath of testPaths){let fileUri=vscode3.Uri.joinPath(workspaceFolders[0].uri,testPath);tests.push({path:testPath,scheme:fileUri.scheme,fsPath:fileUri.fsPath,authority:fileUri.authority,valid:fileUri.scheme==="file"&&fileUri.fsPath.length>0}),fileUri.scheme!=="file"&&issues.push(`WARNING: Non-file URI scheme for ${testPath}: ${fileUri.scheme}`),(fileUri.fsPath.includes("\\\\")||fileUri.fsPath.includes("//"))&&issues.push(`WARNING: Potential path separator issues in ${testPath}`)}return{status:issues.length>0?"ISSUES_FOUND":"OK",tests,issues}}};__name(_DecorationDiagnostics,"DecorationDiagnostics");var DecorationDiagnostics=_DecorationDiagnostics;module2.exports={DecorationDiagnostics}}});var require_decorationTester=__commonJS({"src/decorationTester.js"(exports2,module2){var vscode3=require("vscode"),{getFileName:getFileName2}=require_pathUtils();async function testVSCodeDecorationRendering(){let logger2=require_logger().getLogger();logger2.info("\u{1F3A8} Testing VS Code decoration rendering...");let _TestDecorationProvider=class _TestDecorationProvider{constructor(){this._onDidChangeFileDecorations=new vscode3.EventEmitter,this.onDidChangeFileDecorations=this._onDidChangeFileDecorations.event}provideFileDecoration(uri){let fileName=getFileName2(uri.fsPath||uri.path),decoration=new vscode3.FileDecoration("TEST");return decoration.tooltip=`Test decoration for ${fileName}`,decoration.color=new vscode3.ThemeColor("charts.red"),logger2.info(`\u{1F9EA} Test provider returning decoration for: ${fileName}`),console.log(`\u{1F9EA} TEST DECORATION: ${fileName} \u2192 "TEST"`),decoration}};__name(_TestDecorationProvider,"TestDecorationProvider");let TestDecorationProvider=_TestDecorationProvider,testProvider=new TestDecorationProvider,disposable=vscode3.window.registerFileDecorationProvider(testProvider);return logger2.info("\u{1F9EA} Test decoration provider registered"),setTimeout(()=>{testProvider._onDidChangeFileDecorations.fire(void 0),logger2.info("\u{1F504} Test decoration refresh triggered"),setTimeout(()=>{disposable.dispose(),logger2.info("\u{1F9EA} Test decoration provider disposed")},1e4)},1e3),"Test decoration provider registered for 10 seconds"}__name(testVSCodeDecorationRendering,"testVSCodeDecorationRendering");async function testFileDecorationAPI(){let logger2=require_logger().getLogger();logger2.info("\u{1F527} Testing FileDecoration API...");let tests=[];try{let minimal=new vscode3.FileDecoration("MIN");tests.push({name:"Minimal decoration",success:!0,badge:minimal.badge}),logger2.info("\u2705 Minimal decoration created successfully")}catch(error){tests.push({name:"Minimal decoration",success:!1,error:error.message}),logger2.error("\u274C Minimal decoration failed:",error)}try{let full=new vscode3.FileDecoration("FULL","Full decoration tooltip",new vscode3.ThemeColor("charts.blue"));full.propagate=!1,tests.push({name:"Full decoration",success:!0,badge:full.badge,hasTooltip:!!full.tooltip,hasColor:!!full.color,propagate:full.propagate}),logger2.info("\u2705 Full decoration created successfully")}catch(error){tests.push({name:"Full decoration",success:!1,error:error.message}),logger2.error("\u274C Full decoration failed:",error)}let themeColors=["charts.red","charts.blue","charts.green","charts.yellow","terminal.ansiRed","terminal.ansiGreen","terminal.ansiBlue","editorError.foreground","editorWarning.foreground","editorInfo.foreground"];for(let colorName of themeColors)try{tests.push({name:`ThemeColor: ${colorName}`,success:!0,colorId:colorName})}catch(error){tests.push({name:`ThemeColor: ${colorName}`,success:!1,error:error.message}),logger2.error(`\u274C ThemeColor ${colorName} failed:`,error)}return tests}__name(testFileDecorationAPI,"testFileDecorationAPI");module2.exports={testVSCodeDecorationRendering,testFileDecorationAPI}}});var require_analysisCommands=__commonJS({"src/commands/analysisCommands.js"(exports2,module2){var vscode3=require("vscode"),{fileSystem:fileSystem3}=require_FileSystemAdapter(),{getFileName:getFileName2,getRelativePath:getRelativePath2}=require_pathUtils();function registerAnalysisCommands2({context,fileDateProvider:fileDateProvider2,logger:logger2,generators}){let{generateWorkspaceActivityHTML:generateWorkspaceActivityHTML2,generatePerformanceAnalyticsHTML:generatePerformanceAnalyticsHTML2,generateDiagnosticsHTML:generateDiagnosticsHTML2,generateDiagnosticsWebview:generateDiagnosticsWebview2}=generators,subscriptions=[];subscriptions.push(vscode3.commands.registerCommand("explorerDates.showWorkspaceActivity",async()=>{try{let panel=vscode3.window.createWebviewPanel("workspaceActivity","Workspace File Activity",vscode3.ViewColumn.One,{enableScripts:!0});if(!vscode3.workspace.workspaceFolders){vscode3.window.showWarningMessage("No workspace folder open");return}let workspaceFolder=vscode3.workspace.workspaceFolders[0],files=[],allFiles=await vscode3.workspace.findFiles("**/*","**/node_modules/**",100);for(let fileUri of allFiles)try{let stat=await fileSystem3.stat(fileUri);(typeof stat.isFile=="function"?stat.isFile():!0)&&files.push({path:getRelativePath2(workspaceFolder.uri.fsPath||workspaceFolder.uri.path,fileUri.fsPath||fileUri.path),modified:stat.mtime instanceof Date?stat.mtime:new Date(stat.mtime),size:stat.size})}catch{}files.sort((a,b)=>b.modified.getTime()-a.modified.getTime()),panel.webview.html=generateWorkspaceActivityHTML2(files.slice(0,50)),logger2.info("Workspace activity panel opened")}catch(error){logger2.error("Failed to show workspace activity",error),vscode3.window.showErrorMessage(`Failed to show workspace activity: ${error.message}`)}})),subscriptions.push(vscode3.commands.registerCommand("explorerDates.showPerformanceAnalytics",async()=>{try{let panel=vscode3.window.createWebviewPanel("performanceAnalytics","Explorer Dates Performance Analytics",vscode3.ViewColumn.One,{enableScripts:!0}),metrics=fileDateProvider2?fileDateProvider2.getMetrics():{};panel.webview.html=generatePerformanceAnalyticsHTML2(metrics),logger2.info("Performance analytics panel opened")}catch(error){logger2.error("Failed to show performance analytics",error),vscode3.window.showErrorMessage(`Failed to show performance analytics: ${error.message}`)}})),subscriptions.push(vscode3.commands.registerCommand("explorerDates.debugCache",async()=>{try{if(fileDateProvider2){let metrics=fileDateProvider2.getMetrics(),debugInfo={"Cache Summary":{"Memory Cache Size":metrics.cacheSize,"Cache Hit Rate":metrics.cacheHitRate,"Total Hits":metrics.cacheHits,"Total Misses":metrics.cacheMisses,"Cache Timeout":`${metrics.cacheDebugging.cacheTimeout}ms`},"Advanced Cache":metrics.advancedCache||"Not available","Sample Cache Keys":metrics.cacheDebugging.memoryCacheKeys||[]};vscode3.window.showInformationMessage(`Cache Debug Info:
${JSON.stringify(debugInfo,null,2)}`,{modal:!0}),logger2.info("Cache debug info displayed",debugInfo)}}catch(error){logger2.error("Failed to show cache debug info",error),vscode3.window.showErrorMessage(`Failed to show cache debug info: ${error.message}`)}})),subscriptions.push(vscode3.commands.registerCommand("explorerDates.runDiagnostics",async()=>{try{let config=vscode3.workspace.getConfiguration("explorerDates"),activeEditor=vscode3.window.activeTextEditor,diagnosticResults={"Extension Status":{"Provider Active":fileDateProvider2?"Yes":"No","Decorations Enabled":config.get("showDateDecorations",!0)?"Yes":"No","VS Code Version":vscode3.version,"Extension Version":context.extension.packageJSON.version}};if(activeEditor){let{uri}=activeEditor.document;uri.scheme==="file"&&(diagnosticResults["Current File"]={"File Path":uri.fsPath,"File Extension":getFileName2(uri.fsPath||uri.path).split(".").pop()||"No extension","Is Excluded":fileDateProvider2?await fileDateProvider2._isExcludedSimple(uri):"Unknown"})}if(diagnosticResults.Configuration={"Excluded Folders":config.get("excludedFolders",[]),"Excluded Patterns":config.get("excludedPatterns",[]),"Color Scheme":config.get("colorScheme","none"),"Cache Timeout":`${config.get("cacheTimeout",3e4)}ms`},fileDateProvider2){let metrics=fileDateProvider2.getMetrics();diagnosticResults.Performance={"Total Decorations":metrics.totalDecorations,"Cache Size":metrics.cacheSize,Errors:metrics.errors}}let panel=vscode3.window.createWebviewPanel("explorerDatesDiagnostics","Explorer Dates Diagnostics",vscode3.ViewColumn.One,{enableScripts:!0});panel.webview.html=generateDiagnosticsHTML2(diagnosticResults),logger2.info("Diagnostics panel opened",diagnosticResults)}catch(error){logger2.error("Failed to run diagnostics",error),vscode3.window.showErrorMessage(`Failed to run diagnostics: ${error.message}`)}})),subscriptions.push(vscode3.commands.registerCommand("explorerDates.testDecorations",async()=>{try{logger2.info("\u{1F50D} Starting comprehensive decoration diagnostics...");let{DecorationDiagnostics}=require_decorationDiagnostics(),results=await new DecorationDiagnostics(fileDateProvider2).runComprehensiveDiagnostics(),panel=vscode3.window.createWebviewPanel("decorationDiagnostics","Decoration Diagnostics - Root Cause Analysis",vscode3.ViewColumn.One,{enableScripts:!0});panel.webview.html=generateDiagnosticsWebview2(results);let criticalIssues=[],warnings=[];Object.values(results.tests).forEach(test=>{test.issues&&test.issues.forEach(issue=>{issue.startsWith("CRITICAL:")?criticalIssues.push(issue):issue.startsWith("WARNING:")&&warnings.push(issue)})}),criticalIssues.length>0?vscode3.window.showErrorMessage(`CRITICAL ISSUES FOUND: ${criticalIssues.join(", ")}`):warnings.length>0?vscode3.window.showWarningMessage(`Warnings found: ${warnings.length} potential issues detected. Check diagnostics panel.`):vscode3.window.showInformationMessage("No critical issues found. Decorations should be working properly."),logger2.info("\u{1F50D} Comprehensive diagnostics completed",results)}catch(error){logger2.error("Failed to run comprehensive diagnostics",error),vscode3.window.showErrorMessage(`Diagnostics failed: ${error.message}`)}})),subscriptions.push(vscode3.commands.registerCommand("explorerDates.monitorDecorations",async()=>{if(!fileDateProvider2){vscode3.window.showErrorMessage("Decoration provider not available");return}fileDateProvider2.startProviderCallMonitoring(),fileDateProvider2.forceRefreshAllDecorations(),setTimeout(()=>{let stats=fileDateProvider2.getProviderCallStats(),message=`VS Code Decoration Requests: ${stats.totalCalls} calls for ${stats.uniqueFiles} files`;vscode3.window.showInformationMessage(message),logger2.info("\u{1F50D} Decoration monitoring results:",stats)},5e3),vscode3.window.showInformationMessage("Started monitoring VS Code decoration requests. Results in 5 seconds...")})),subscriptions.push(vscode3.commands.registerCommand("explorerDates.testVSCodeRendering",async()=>{try{let{testVSCodeDecorationRendering,testFileDecorationAPI}=require_decorationTester();logger2.info("\u{1F3A8} Testing VS Code decoration rendering system...");let apiTests=await testFileDecorationAPI();logger2.info("\u{1F527} FileDecoration API tests:",apiTests);let renderResult=await testVSCodeDecorationRendering();logger2.info("\u{1F3A8} Decoration rendering test:",renderResult),vscode3.window.showInformationMessage('VS Code decoration rendering test started. Check Output panel and Explorer for "TEST" badges on files.')}catch(error){logger2.error("Failed to test VS Code rendering:",error),vscode3.window.showErrorMessage(`VS Code rendering test failed: ${error.message}`)}})),subscriptions.push(vscode3.commands.registerCommand("explorerDates.quickFix",async()=>{try{let config=vscode3.workspace.getConfiguration("explorerDates"),fixes=[];config.get("showDateDecorations",!0)||fixes.push({issue:"Date decorations are disabled",description:"Enable date decorations",fix:__name(async()=>config.update("showDateDecorations",!0,vscode3.ConfigurationTarget.Global),"fix")});let excludedPatterns=config.get("excludedPatterns",[]);if(excludedPatterns.includes("**/*")&&fixes.push({issue:"All files are excluded by pattern",description:"Remove overly broad exclusion pattern",fix:__name(async()=>{let newPatterns=excludedPatterns.filter(p=>p!=="**/*");await config.update("excludedPatterns",newPatterns,vscode3.ConfigurationTarget.Global)},"fix")}),fixes.length===0){vscode3.window.showInformationMessage("No common issues detected. Decorations should be working.");return}let selected=await vscode3.window.showQuickPick(fixes.map(fix=>({label:fix.description,description:fix.issue,fix:fix.fix})),{placeHolder:"Select an issue to fix automatically"});selected&&(await selected.fix(),vscode3.window.showInformationMessage("Fixed! Try refreshing decorations now."),fileDateProvider2&&(fileDateProvider2.clearAllCaches(),fileDateProvider2.refreshAll()))}catch(error){logger2.error("Failed to run quick fix",error),vscode3.window.showErrorMessage(`Failed to run quick fix: ${error.message}`)}})),subscriptions.push(vscode3.commands.registerCommand("explorerDates.showKeyboardShortcuts",async()=>{try{fileDateProvider2!=null&&fileDateProvider2._accessibility?await fileDateProvider2._accessibility.showKeyboardShortcutsHelp():vscode3.window.showInformationMessage("Keyboard shortcuts: Ctrl+Shift+D (toggle), Ctrl+Shift+C (copy date), Ctrl+Shift+I (file details), Ctrl+Shift+R (refresh), Ctrl+Shift+A (workspace activity)"),logger2.info("Keyboard shortcuts help shown")}catch(error){logger2.error("Failed to show keyboard shortcuts help",error),vscode3.window.showErrorMessage(`Failed to show keyboard shortcuts help: ${error.message}`)}})),subscriptions.forEach(disposable=>context.subscriptions.push(disposable))}__name(registerAnalysisCommands2,"registerAnalysisCommands");module2.exports={registerAnalysisCommands:registerAnalysisCommands2}}});var require_onboardingCommands=__commonJS({"src/commands/onboardingCommands.js"(exports2,module2){var vscode3=require("vscode");function registerOnboardingCommands2({context,logger:logger2,getOnboardingManager}){let subscriptions=[];subscriptions.push(vscode3.commands.registerCommand("explorerDates.showFeatureTour",async()=>{try{await getOnboardingManager().showFeatureTour(),logger2.info("Feature tour opened")}catch(error){logger2.error("Failed to show feature tour",error),vscode3.window.showErrorMessage(`Failed to show feature tour: ${error.message}`)}})),subscriptions.push(vscode3.commands.registerCommand("explorerDates.showQuickSetup",async()=>{try{await getOnboardingManager().showQuickSetupWizard(),logger2.info("Quick setup wizard opened")}catch(error){logger2.error("Failed to show quick setup wizard",error),vscode3.window.showErrorMessage(`Failed to show quick setup wizard: ${error.message}`)}})),subscriptions.push(vscode3.commands.registerCommand("explorerDates.showWhatsNew",async()=>{try{let extensionVersion=context.extension.packageJSON.version;await getOnboardingManager().showWhatsNew(extensionVersion),logger2.info("What's new panel opened")}catch(error){logger2.error("Failed to show what's new",error),vscode3.window.showErrorMessage(`Failed to show what's new: ${error.message}`)}})),subscriptions.forEach(disposable=>context.subscriptions.push(disposable))}__name(registerOnboardingCommands2,"registerOnboardingCommands");module2.exports={registerOnboardingCommands:registerOnboardingCommands2}}});var require_onboarding=__commonJS({"src/onboarding.js"(exports2,module2){var vscode3=require("vscode"),{getLogger:getLogger2}=require_logger(),{getLocalization:getLocalization2}=require_localization(),_OnboardingManager=class _OnboardingManager{constructor(context){this._context=context,this._logger=getLogger2(),this._l10n=getLocalization2(),this._hasShownWelcome=context.globalState.get("explorerDates.hasShownWelcome",!1),this._hasCompletedSetup=context.globalState.get("explorerDates.hasCompletedSetup",!1),this._onboardingVersion=context.globalState.get("explorerDates.onboardingVersion","0.0.0"),this._logger.info("OnboardingManager initialized",{hasShownWelcome:this._hasShownWelcome,hasCompletedSetup:this._hasCompletedSetup,onboardingVersion:this._onboardingVersion})}async shouldShowOnboarding(){let extensionVersion=this._context.extension.packageJSON.version;return!this._hasShownWelcome||!this._hasCompletedSetup||this._shouldShowVersionUpdate(extensionVersion)}_shouldShowVersionUpdate(currentVersion){if(this._onboardingVersion==="0.0.0")return!0;let[currentMajor]=currentVersion.split(".").map(Number),[savedMajor]=this._onboardingVersion.split(".").map(Number);return currentMajor>savedMajor}_isMinorUpdate(currentVersion){if(this._onboardingVersion==="0.0.0")return!1;let[currentMajor,currentMinor]=currentVersion.split(".").map(Number),[savedMajor,savedMinor]=this._onboardingVersion.split(".").map(Number);return currentMajor===savedMajor&&currentMinor>savedMinor}async showWelcomeMessage(){try{let extensionVersion=this._context.extension.packageJSON.version,isUpdate=this._hasShownWelcome,isMinorUpdate=this._isMinorUpdate(extensionVersion);if(isMinorUpdate)return this._showGentleUpdateNotification(extensionVersion);let message=isUpdate?`Explorer Dates has been updated to v${extensionVersion} with new features and improvements!`:"See file modification dates right in VS Code Explorer with intuitive time badges, file sizes, Git info, and much more!",actions=isUpdate?["\u{1F4D6} What's New","\u2699\uFE0F Settings","Dismiss"]:["\u{1F680} Quick Setup","\u{1F4D6} Feature Tour","\u2699\uFE0F Settings","Maybe Later"],action=await vscode3.window.showInformationMessage(message,{modal:!1},...actions);switch(await this._context.globalState.update("explorerDates.hasShownWelcome",!0),await this._context.globalState.update("explorerDates.onboardingVersion",extensionVersion),action){case"\u{1F680} Quick Setup":await this.showQuickSetupWizard();break;case"\u{1F4D6} Feature Tour":await this.showFeatureTour();break;case"\u{1F4D6} What's New":await this.showWhatsNew(extensionVersion);break;case"\u2699\uFE0F Settings":await vscode3.commands.executeCommand("workbench.action.openSettings","explorerDates");break;case"previewConfiguration":await vscode3.commands.executeCommand("explorerDates.previewConfiguration",message.settings);break;case"clearPreview":await vscode3.commands.executeCommand("explorerDates.clearPreview");break}this._logger.info("Welcome message shown",{action,isUpdate,isMinorUpdate})}catch(error){this._logger.error("Failed to show welcome message",error)}}async _showGentleUpdateNotification(version){let statusBarItem=vscode3.window.createStatusBarItem(vscode3.StatusBarAlignment.Right,100);statusBarItem.text=`$(check) Explorer Dates updated to v${version}`,statusBarItem.tooltip="Click to see what's new in Explorer Dates",statusBarItem.command="explorerDates.showWhatsNew",statusBarItem.show(),setTimeout(()=>{statusBarItem.dispose()},1e4),await this._context.globalState.update("explorerDates.onboardingVersion",version),this._logger.info("Showed gentle update notification",{version})}async showQuickSetupWizard(){try{let panel=vscode3.window.createWebviewPanel("explorerDatesSetup","Explorer Dates Quick Setup",vscode3.ViewColumn.One,{enableScripts:!0,retainContextWhenHidden:!0});panel.webview.html=this._generateSetupWizardHTML(),panel.webview.onDidReceiveMessage(async message=>{await this._handleSetupWizardMessage(message,panel)}),this._logger.info("Quick setup wizard opened")}catch(error){this._logger.error("Failed to show setup wizard",error)}}async _handleSetupWizardMessage(message,panel){try{switch(message.command){case"applyConfiguration":await this._applyQuickConfiguration(message.configuration),await this._context.globalState.update("explorerDates.hasCompletedSetup",!0),vscode3.window.showInformationMessage("\u2705 Explorer Dates configured successfully!"),panel.dispose();break;case"previewConfiguration":message.settings&&(await vscode3.commands.executeCommand("explorerDates.previewConfiguration",message.settings),this._logger.info("Configuration preview applied via webview",message.settings));break;case"clearPreview":await vscode3.commands.executeCommand("explorerDates.clearPreview"),this._logger.info("Configuration preview cleared via webview");break;case"skipSetup":await this._context.globalState.update("explorerDates.hasCompletedSetup",!0),panel.dispose();break;case"openSettings":await vscode3.commands.executeCommand("workbench.action.openSettings","explorerDates"),panel.dispose();break}}catch(error){this._logger.error("Failed to handle setup wizard message",error)}}async _applyQuickConfiguration(configuration){let config=vscode3.workspace.getConfiguration("explorerDates");if(configuration.preset){let preset=this._getConfigurationPresets()[configuration.preset];if(preset){this._logger.info(`Applying preset: ${configuration.preset}`,preset.settings);for(let[key,value]of Object.entries(preset.settings))await config.update(key,value,vscode3.ConfigurationTarget.Global),this._logger.debug(`Updated setting: explorerDates.${key} = ${value}`);this._logger.info(`Applied preset: ${configuration.preset}`,preset.settings),vscode3.window.showInformationMessage(`Applied "${preset.name}" configuration. Changes should be visible immediately!`)}}if(configuration.individual){for(let[key,value]of Object.entries(configuration.individual))await config.update(key,value,vscode3.ConfigurationTarget.Global);this._logger.info("Applied individual settings",configuration.individual)}try{await vscode3.commands.executeCommand("explorerDates.refreshDateDecorations"),this._logger.info("Decorations refreshed after configuration change")}catch(error){this._logger.warn("Failed to refresh decorations after configuration change",error)}}_getConfigurationPresets(){return{minimal:{name:"Minimal",description:"Clean and simple - just show modification times in short format",settings:{dateDecorationFormat:"relative-short",colorScheme:"none",highContrastMode:!1,showFileSize:!1,showGitInfo:"none",badgePriority:"time",fadeOldFiles:!1,enableContextMenu:!1,showStatusBar:!1}},developer:{name:"Developer",description:"Perfect for development - includes Git info, file sizes, and color coding",settings:{dateDecorationFormat:"smart",colorScheme:"recency",highContrastMode:!1,showFileSize:!0,fileSizeFormat:"auto",showGitInfo:"author",badgePriority:"time",fadeOldFiles:!0,fadeThreshold:30,enableContextMenu:!0,showStatusBar:!0}},powerUser:{name:"Power User",description:"Maximum information - all features enabled with vibrant colors",settings:{dateDecorationFormat:"smart",colorScheme:"vibrant",highContrastMode:!1,showFileSize:!0,fileSizeFormat:"auto",showGitInfo:"both",badgePriority:"time",fadeOldFiles:!0,fadeThreshold:14,enableContextMenu:!0,showStatusBar:!0,smartExclusions:!0,progressiveLoading:!0,persistentCache:!0}},gitFocused:{name:"Git-Focused",description:"Show author initials as badges with full Git information in tooltips",settings:{dateDecorationFormat:"smart",colorScheme:"file-type",highContrastMode:!1,showFileSize:!1,showGitInfo:"both",badgePriority:"author",fadeOldFiles:!1,enableContextMenu:!0,showStatusBar:!0}},accessible:{name:"Accessible",description:"High contrast and screen reader friendly with detailed tooltips",settings:{dateDecorationFormat:"relative-short",colorScheme:"none",highContrastMode:!0,accessibilityMode:!0,showFileSize:!1,showGitInfo:"none",badgePriority:"time",fadeOldFiles:!1,enableContextMenu:!0,keyboardNavigation:!0}}}}async showFeatureTour(){try{let panel=vscode3.window.createWebviewPanel("explorerDatesFeatureTour","Explorer Dates Feature Tour",vscode3.ViewColumn.One,{enableScripts:!0,retainContextWhenHidden:!0});panel.webview.html=this._generateFeatureTourHTML(),panel.webview.onDidReceiveMessage(async message=>{message.command==="openSettings"?await vscode3.commands.executeCommand("workbench.action.openSettings",message.setting||"explorerDates"):message.command==="runCommand"&&await vscode3.commands.executeCommand(message.commandId)}),this._logger.info("Feature tour opened")}catch(error){this._logger.error("Failed to show feature tour",error)}}_generateSetupWizardHTML(){let allPresets=this._getConfigurationPresets(),simplifiedPresets={minimal:allPresets.minimal,developer:allPresets.developer,accessible:allPresets.accessible};return`
            <!DOCTYPE html>
            <html>
            <head>
                <meta charset="UTF-8">
...
${selectedTip.description}`;await vscode3.window.showInformationMessage(message,"Show More Tips","Got it!")==="Show More Tips"&&await this.showFeatureTour()}async showWhatsNew(version){try{let panel=vscode3.window.createWebviewPanel("explorerDatesWhatsNew",`Explorer Dates v${version} - What's New`,vscode3.ViewColumn.One,{enableScripts:!0,retainContextWhenHidden:!1});panel.webview.html=this._generateWhatsNewHTML(version),panel.webview.onDidReceiveMessage(async message=>{switch(message.command){case"openSettings":await vscode3.commands.executeCommand("workbench.action.openSettings","explorerDates"),panel.dispose();break;case"tryFeature":message.feature==="badgePriority"&&(await vscode3.workspace.getConfiguration("explorerDates").update("badgePriority","author",vscode3.ConfigurationTarget.Global),vscode3.window.showInformationMessage("Badge priority set to author! You should see author initials on files now."));break;case"dismiss":panel.dispose();break}})}catch(error){this._logger.error("Failed to show what's new",error)}}_generateWhatsNewHTML(version){return`
            <!DOCTYPE html>
            <html lang="en">
            <head>
                <meta charset="UTF-8">
        ...
        `}};__name(_OnboardingManager,"OnboardingManager");var OnboardingManager=_OnboardingManager;module2.exports={OnboardingManager}}});var require_workspaceTemplates=__commonJS({"src/workspaceTemplates.js"(exports2,module2){var vscode3=require("vscode"),{getLogger:getLogger2}=require_logger(),{fileSystem:fileSystem3}=require_FileSystemAdapter(),{GLOBAL_STATE_KEYS}=require_constants(),logger2=getLogger2(),_WorkspaceTemplatesManager=class _WorkspaceTemplatesManager{constructor(context){this._context=context,this._storage=(context==null?void 0:context.globalState)||null,this._storageKey=GLOBAL_STATE_KEYS.TEMPLATE_STORE,this._fs=fileSystem3,this.templatesPath=null,this.builtInTemplates=this.getBuiltInTemplates(),logger2.info("Workspace Templates Manager initialized")}_getStoredTemplates(){return this._storage?this._storage.get(this._storageKey,{}):{}}async _saveStoredTemplates(templates){if(!this._storage)throw new Error("Template storage unavailable");await this._storage.update(this._storageKey,templates)}_getTemplate(templateId){return this.builtInTemplates[templateId]?this.builtInTemplates[templateId]:this._getStoredTemplates()[templateId]}getBuiltInTemplates(){return{"web-development":{name:"Web Development",description:"Optimized for web projects with focus on source files",settings:{"explorerDates.showDateDecorations":!0,"explorerDates.dateDecorationFormat":"relative-short","explorerDates.colorScheme":"file-type","explorerDates.showFileSize":!0,"explorerDates.fadeOldFiles":!0,"explorerDates.fadeThreshold":14,"explorerDates.excludedPatterns":["**/node_modules/**","**/dist/**","**/build/**","**/.next/**","**/coverage/**"],"explorerDates.enableContextMenu":!0,"explorerDates.showGitInfo":"author"}},"data-science":{name:"Data Science",description:"Focused on notebooks and data files with detailed timestamps",settings:{"explorerDates.showDateDecorations":!0,"explorerDates.dateDecorationFormat":"absolute-long","explorerDates.colorScheme":"none","explorerDates.showFileSize":!0,"explorerDates.showGitInfo":"none","explorerDates.highContrastMode":!1,"explorerDates.excludedPatterns":["**/__pycache__/**","**/.ipynb_checkpoints/**","**/data/raw/**"],"explorerDates.badgePriority":"size"}},documentation:{name:"Documentation",description:"Clean display for documentation projects",settings:{"explorerDates.showDateDecorations":!0,"explorerDates.dateDecorationFormat":"smart","explorerDates.colorScheme":"subtle","explorerDates.showFileSize":!1,"explorerDates.excludedPatterns":["**/node_modules/**","**/.git/**"],"explorerDates.fadeOldFiles":!1,"explorerDates.enableContextMenu":!1}},enterprise:{name:"Enterprise",description:"Full feature set with Git integration and analytics",settings:{"explorerDates.showDateDecorations":!0,"explorerDates.dateDecorationFormat":"smart","explorerDates.colorScheme":"recency","explorerDates.showFileSize":!0,"explorerDates.showGitInfo":"author","explorerDates.enableContextMenu":!0,"explorerDates.showStatusBar":!0,"explorerDates.smartExclusions":!0,"explorerDates.progressiveLoading":!0,"explorerDates.persistentCache":!0,"explorerDates.enableReporting":!0}},minimal:{name:"Minimal",description:"Clean, distraction-free setup",settings:{"explorerDates.showDateDecorations":!0,"explorerDates.dateDecorationFormat":"relative-short","explorerDates.colorScheme":"none","explorerDates.showFileSize":!1,"explorerDates.badgePriority":"time","explorerDates.enableContextMenu":!1,"explorerDates.progressiveLoading":!1}}}}async saveCurrentConfiguration(templateName,description=""){try{let config=vscode3.workspace.getConfiguration("explorerDates"),settings={},inspect=config.inspect();if(inspect)for(let[key,value]of Object.entries(inspect))value&&typeof value=="object"&&"workspaceValue"in value?settings[`explorerDates.${key}`]=value.workspaceValue:value&&typeof value=="object"&&"globalValue"in value&&(settings[`explorerDates.${key}`]=value.globalValue);let template={name:templateName,description,createdAt:new Date().toISOString(),version:"1.0.0",settings},templateId=templateName.toLowerCase().replace(/[^a-z0-9-]/g,"-"),storedTemplates=this._getStoredTemplates();storedTemplates[templateId]=template,await this._saveStoredTemplates(storedTemplates);let syncPath=config.get("templateSyncPath","");if(syncPath&&!this._fs.isWeb)try{let targetPath=`${syncPath.replace(/[/\\]?$/,"")}/${templateId}.json`;await this._fs.writeFile(targetPath,JSON.stringify(template,null,2)),logger2.info(`Synced template to ${targetPath}`)}catch(syncError){logger2.warn("Failed to sync template to disk path",syncError)}return vscode3.window.showInformationMessage(`Template "${templateName}" saved successfully!`),logger2.info(`Saved workspace template: ${templateName}`),!0}catch(error){return logger2.error("Failed to save template:",error),vscode3.window.showErrorMessage(`Failed to save template: ${error.message}`),!1}}async loadTemplate(templateId){try{let template=this._getTemplate(templateId);if(!template)throw new Error(`Template "${templateId}" not found`);let config=vscode3.workspace.getConfiguration();for(let[key,value]of Object.entries(template.settings))await config.update(key,value,vscode3.ConfigurationTarget.Workspace);return vscode3.window.showInformationMessage(`Template "${template.name}" applied successfully!`),logger2.info(`Applied workspace template: ${template.name}`),!0}catch(error){return logger2.error("Failed to load template:",error),vscode3.window.showErrorMessage(`Failed to load template: ${error.message}`),!1}}async getAvailableTemplates(){let templates=[];for(let[id,template]of Object.entries(this.builtInTemplates))templates.push({id,name:template.name,description:template.description,type:"built-in",createdAt:null});try{let storedTemplates=this._getStoredTemplates();for(let[id,template]of Object.entries(storedTemplates))templates.push({id,name:template.name,description:template.description,type:"custom",createdAt:template.createdAt})}catch(error){logger2.error("Failed to load custom templates:",error)}return templates}async deleteTemplate(templateId){try{if(this.builtInTemplates[templateId])return vscode3.window.showErrorMessage("Cannot delete built-in templates"),!1;let storedTemplates=this._getStoredTemplates();if(!storedTemplates[templateId])throw new Error(`Template "${templateId}" not found`);return delete storedTemplates[templateId],await this._saveStoredTemplates(storedTemplates),vscode3.window.showInformationMessage(`Template "${templateId}" deleted successfully!`),logger2.info(`Deleted workspace template: ${templateId}`),!0}catch(error){return logger2.error("Failed to delete template:",error),vscode3.window.showErrorMessage(`Failed to delete template: ${error.message}`),!1}}async exportTemplate(templateId,exportTarget){try{let template=this._getTemplate(templateId);if(!template)throw new Error(`Template "${templateId}" not found`);let payload=JSON.stringify(template,null,2);if(this._fs.isWeb){let encoded=encodeURIComponent(payload);return await vscode3.env.openExternal(vscode3.Uri.parse(`data:application/json;charset=utf-8,${encoded}`)),vscode3.window.showInformationMessage("Template download triggered in browser"),!0}let targetPath=exportTarget instanceof vscode3.Uri?exportTarget.fsPath:exportTarget;return await this._fs.writeFile(targetPath,payload),vscode3.window.showInformationMessage(`Template exported to ${targetPath}`),logger2.info(`Exported template ${templateId} to ${targetPath}`),!0}catch(error){return logger2.error("Failed to export template:",error),vscode3.window.showErrorMessage(`Failed to export template: ${error.message}`),!1}}async importTemplate(importTarget){try{let target=importTarget instanceof vscode3.Uri?importTarget:vscode3.Uri.file(importTarget),templateData=await this._fs.readFile(target,"utf8"),template=JSON.parse(templateData);if(!template.name||!template.settings)throw new Error("Invalid template format");let templateName=template.name.toLowerCase().replace(/[^a-z0-9-]/g,"-"),storedTemplates=this._getStoredTemplates();return storedTemplates[templateName]=template,await this._saveStoredTemplates(storedTemplates),vscode3.window.showInformationMessage(`Template "${template.name}" imported successfully!`),logger2.info(`Imported template: ${template.name}`),!0}catch(error){return logger2.error("Failed to import template:",error),vscode3.window.showErrorMessage(`Failed to import template: ${error.message}`),!1}}async showTemplateManager(){try{let templates=await this.getAvailableTemplates(),panel=vscode3.window.createWebviewPanel("templateManager","Explorer Dates - Template Manager",vscode3.ViewColumn.One,{enableScripts:!0,retainContextWhenHidden:!0});panel.webview.html=this.getTemplateManagerHtml(templates),panel.webview.onDidReceiveMessage(async message=>{switch(message.command){case"loadTemplate":await this.loadTemplate(message.templateId);break;case"deleteTemplate":{await this.deleteTemplate(message.templateId);let updatedTemplates=await this.getAvailableTemplates();panel.webview.postMessage({command:"refreshTemplates",templates:updatedTemplates});break}case"exportTemplate":{let result=await vscode3.window.showSaveDialog({defaultUri:vscode3.Uri.file(`${message.templateId}.json`),filters:{JSON:["json"]}});result&&await this.exportTemplate(message.templateId,result);break}case"saveConfig":{await this.saveCurrentConfiguration(message.name,message.description);let updatedTemplates=await this.getAvailableTemplates();panel.webview.postMessage({command:"refreshTemplates",templates:updatedTemplates});break}case"importTemplate":{let result=await vscode3.window.showOpenDialog({canSelectMany:!1,filters:{JSON:["json"]}});if(result&&result[0]){await this.importTemplate(result[0]);let updatedTemplates=await this.getAvailableTemplates();panel.webview.postMessage({command:"refreshTemplates",templates:updatedTemplates})}break}}}),logger2.info("Template Manager opened")}catch(error){logger2.error("Failed to show template manager:",error),vscode3.window.showErrorMessage("Failed to open Template Manager")}}getTemplateManagerHtml(templates){return`<!DOCTYPE html>
        <html>
        <head>
            <meta charset="UTF-8">
            <meta name="viewport" content="width=device-width, initial-scale=1.0">
        ...
        </html>`}};__name(_WorkspaceTemplatesManager,"WorkspaceTemplatesManager");var WorkspaceTemplatesManager=_WorkspaceTemplatesManager;module2.exports={WorkspaceTemplatesManager}}});var require_extensionApi=__commonJS({"src/extensionApi.js"(exports2,module2){var vscode3=require("vscode"),{getLogger:getLogger2}=require_logger(),_BaseEventEmitter=class _BaseEventEmitter{constructor(){this._listeners=new Map}on(event,listener){let handlers=this._listeners.get(event)||[];return handlers.push(listener),this._listeners.set(event,handlers),this}off(event,listener){let handlers=this._listeners.get(event);if(!handlers)return this;let index=handlers.indexOf(listener);return index!==-1&&handlers.splice(index,1),this}emit(event,...args){let handlers=this._listeners.get(event);return handlers&&handlers.slice().forEach(handler=>{try{handler(...args)}catch(error){console.error(`Explorer Dates API handler failed for "${event}":`,error)}}),this}};__name(_BaseEventEmitter,"BaseEventEmitter");var BaseEventEmitter=_BaseEventEmitter,logger2=getLogger2(),_ExtensionApiManager=class _ExtensionApiManager extends BaseEventEmitter{constructor(){super(),this.plugins=new Map,this.api=null,this.decorationProviders=new Map,this.initialize(),this._setupConfigurationListener()}initialize(){this.api=this.createPublicApi(),logger2.info("Extension API Manager initialized")}createPublicApi(){return{getFileDecorations:this.getFileDecorations.bind(this),refreshDecorations:this.refreshDecorations.bind(this),registerPlugin:this.registerPlugin.bind(this),unregisterPlugin:this.unregisterPlugin.bind(this),registerDecorationProvider:this.registerDecorationProvider.bind(this),unregisterDecorationProvider:this.unregisterDecorationProvider.bind(this),onDecorationChanged:this.onDecorationChanged.bind(this),onFileScanned:this.onFileScanned.bind(this),formatDate:this.formatDate.bind(this),getFileStats:this.getFileStats.bind(this),version:"1.1.0",apiVersion:"1.0.0"}}async getFileDecorations(filePaths){if(!this._isApiUsable("getFileDecorations"))return[];try{let decorations=[];for(let filePath of filePaths){let uri=vscode3.Uri.file(filePath),decoration=await this.getDecorationForFile(uri);decoration&&decorations.push({uri:uri.toString(),decoration})}return decorations}catch(error){return logger2.error("Failed to get file decorations:",error),[]}}async getDecorationForFile(uri){if(!this._isApiUsable("getDecorationForFile"))return null;try{let stat=await vscode3.workspace.fs.stat(uri),lastModified=new Date(stat.mtime),decoration={badge:this.formatDate(lastModified,"smart"),color:void 0,tooltip:`Modified: ${lastModified.toLocaleString()}`};for(let[providerId,provider]of this.decorationProviders)try{let customDecoration=await provider.provideDecoration(uri,stat,decoration);customDecoration&&(decoration={...decoration,...customDecoration})}catch(error){logger2.error(`Decoration provider ${providerId} failed:`,error)}return decoration}catch(error){return logger2.error("Failed to get decoration for file:",error),null}}async refreshDecorations(filePaths=null){if(!this._isApiUsable("refreshDecorations"))return!1;try{return this.emit("decorationRefreshRequested",filePaths),logger2.info("Decoration refresh requested"),!0}catch(error){return logger2.error("Failed to refresh decorations:",error),!1}}registerPlugin(pluginId,plugin){if(!this._canUsePlugins(`registerPlugin:${pluginId}`))return!1;try{if(!this.validatePlugin(plugin))throw new Error("Invalid plugin structure");return this.plugins.set(pluginId,{...plugin,registeredAt:new Date,active:!0}),typeof plugin.activate=="function"&&plugin.activate(this.api),this.emit("pluginRegistered",{pluginId,plugin}),logger2.info(`Plugin registered: ${pluginId}`),!0}catch(error){return logger2.error(`Failed to register plugin ${pluginId}:`,error),!1}}unregisterPlugin(pluginId){if(!this._canUsePlugins(`unregisterPlugin:${pluginId}`))return!1;try{let plugin=this.plugins.get(pluginId);return plugin?(typeof plugin.deactivate=="function"&&plugin.deactivate(),this.plugins.delete(pluginId),this.emit("pluginUnregistered",{pluginId}),logger2.info(`Plugin unregistered: ${pluginId}`),!0):!1}catch(error){return logger2.error(`Failed to unregister plugin ${pluginId}:`,error),!1}}registerDecorationProvider(providerId,provider){if(!this._canUsePlugins(`registerDecorationProvider:${providerId}`))return!1;try{if(!this.validateDecorationProvider(provider))throw new Error("Invalid decoration provider");return this.decorationProviders.set(providerId,provider),this.emit("decorationProviderRegistered",{providerId,provider}),logger2.info(`Decoration provider registered: ${providerId}`),!0}catch(error){return logger2.error(`Failed to register decoration provider ${providerId}:`,error),!1}}unregisterDecorationProvider(providerId){if(!this._canUsePlugins(`unregisterDecorationProvider:${providerId}`))return!1;try{let removed=this.decorationProviders.delete(providerId);return removed&&(this.emit("decorationProviderUnregistered",{providerId}),logger2.info(`Decoration provider unregistered: ${providerId}`)),removed}catch(error){return logger2.error(`Failed to unregister decoration provider ${providerId}:`,error),!1}}onDecorationChanged(callback){return this.on("decorationChanged",callback),()=>this.off("decorationChanged",callback)}onFileScanned(callback){return this.on("fileScanned",callback),()=>this.off("fileScanned",callback)}formatDate(date,format=null){if(!this._isApiUsable("formatDate"))return"";try{let config=vscode3.workspace.getConfiguration("explorerDates"),displayFormat=format||config.get("displayFormat","smart"),diffMs=new Date-date,diffDays=Math.floor(diffMs/(1e3*60*60*24));switch(displayFormat){case"relative-short":return this.getRelativeTimeShort(diffMs);case"relative-long":return this.getRelativeTimeLong(diffMs);case"absolute-short":return date.toLocaleDateString("en-US",{month:"short",day:"numeric"});case"absolute-long":return date.toLocaleDateString("en-US",{year:"numeric",month:"long",day:"numeric"});case"smart":default:return diffDays<7?this.getRelativeTimeShort(diffMs):date.toLocaleDateString("en-US",{month:"short",day:"numeric"})}}catch(error){return logger2.error("Failed to format date:",error),date.toLocaleDateString()}}async getFileStats(filePath){if(!this._isApiUsable("getFileStats"))return null;try{let uri=vscode3.Uri.file(filePath),stat=await vscode3.workspace.fs.stat(uri);return{path:filePath,size:stat.size,created:new Date(stat.ctime),modified:new Date(stat.mtime),type:stat.type===vscode3.FileType.Directory?"directory":"file"}}catch(error){return logger2.error("Failed to get file stats:",error),null}}getApi(){return this.api}getRegisteredPlugins(){let plugins=[];for(let[id,plugin]of this.plugins)plugins.push({id,name:plugin.name,version:plugin.version,author:plugin.author,active:plugin.active,registeredAt:plugin.registeredAt});return plugins}validatePlugin(plugin){return!(!plugin||typeof plugin!="object"||!plugin.name||!plugin.version||plugin.activate&&typeof plugin.activate!="function"||plugin.deactivate&&typeof plugin.deactivate!="function")}validateDecorationProvider(provider){return!(!provider||typeof provider!="object"||typeof provider.provideDecoration!="function")}getRelativeTimeShort(diffMs){let diffSeconds=Math.floor(diffMs/1e3),diffMinutes=Math.floor(diffSeconds/60),diffHours=Math.floor(diffMinutes/60),diffDays=Math.floor(diffHours/24);if(diffSeconds<60)return`${diffSeconds}s`;if(diffMinutes<60)return`${diffMinutes}m`;if(diffHours<24)return`${diffHours}h`;if(diffDays<30)return`${diffDays}d`;let diffMonths=Math.floor(diffDays/30);return diffMonths<12?`${diffMonths}mo`:`${Math.floor(diffMonths/12)}y`}getRelativeTimeLong(diffMs){let diffSeconds=Math.floor(diffMs/1e3),diffMinutes=Math.floor(diffSeconds/60),diffHours=Math.floor(diffMinutes/60),diffDays=Math.floor(diffHours/24);if(diffSeconds<60)return`${diffSeconds} second${diffSeconds!==1?"s":""} ago`;if(diffMinutes<60)return`${diffMinutes} minute${diffMinutes!==1?"s":""} ago`;if(diffHours<24)return`${diffHours} hour${diffHours!==1?"s":""} ago`;if(diffDays<30)return`${diffDays} day${diffDays!==1?"s":""} ago`;let diffMonths=Math.floor(diffDays/30);if(diffMonths<12)return`${diffMonths} month${diffMonths!==1?"s":""} ago`;let diffYears=Math.floor(diffMonths/12);return`${diffYears} year${diffYears!==1?"s":""} ago`}getColorForAge(date){if(!vscode3.workspace.getConfiguration("explorerDates").get("colorCoding",!1))return;let diffHours=(new Date-date)/(1e3*60*60);return diffHours<1?new vscode3.ThemeColor("charts.green"):diffHours<24?new vscode3.ThemeColor("charts.yellow"):diffHours<168?new vscode3.ThemeColor("charts.orange"):new vscode3.ThemeColor("charts.red")}createExamplePlugin(){return{name:"File Size Display",version:"1.0.0",author:"Explorer Dates",description:"Adds file size to decorations",activate:__name(api=>{api.registerDecorationProvider("fileSize",{provideDecoration:__name(async(uri,stat,currentDecoration)=>{let size=this.formatFileSize(stat.size);return{badge:`${currentDecoration.badge} \u2022 ${size}`,tooltip:`${currentDecoration.tooltip}
Size: ${size}`}},"provideDecoration")}),console.log("File Size Display plugin activated")},"activate"),deactivate:__name(()=>{console.log("File Size Display plugin deactivated")},"deactivate")}}_setupConfigurationListener(){vscode3.workspace.onDidChangeConfiguration(event=>{(event.affectsConfiguration("explorerDates.enableExtensionApi")||event.affectsConfiguration("explorerDates.allowExternalPlugins"))&&logger2.info("Explorer Dates API configuration changed",{apiEnabled:this._isApiEnabled(),externalPluginsAllowed:this._allowsExternalPlugins()})})}_isApiEnabled(){return vscode3.workspace.getConfiguration("explorerDates").get("enableExtensionApi",!0)}_allowsExternalPlugins(){return vscode3.workspace.getConfiguration("explorerDates").get("allowExternalPlugins",!0)}_isApiUsable(featureName){return this._isApiEnabled()?!0:(logger2.warn(`Explorer Dates API request "${featureName}" ignored because enableExtensionApi is disabled.`),!1)}_canUsePlugins(featureName){return this._isApiUsable(featureName)?this._allowsExternalPlugins()?!0:(logger2.warn(`Explorer Dates plugin request "${featureName}" ignored because allowExternalPlugins is disabled.`),!1):!1}formatFileSize(bytes){if(bytes===0)return"0 B";let k=1024,sizes=["B","KB","MB","GB"],i=Math.floor(Math.log(bytes)/Math.log(k));return parseFloat((bytes/Math.pow(k,i)).toFixed(1))+" "+sizes[i]}};__name(_ExtensionApiManager,"ExtensionApiManager");var ExtensionApiManager=_ExtensionApiManager;module2.exports={ExtensionApiManager}}});var require_exportReporting=__commonJS({"src/exportReporting.js"(exports2,module2){var vscode3=require("vscode"),{getLogger:getLogger2}=require_logger(),{fileSystem:fileSystem3}=require_FileSystemAdapter(),{getExtension,normalizePath}=require_pathUtils(),logger2=getLogger2(),isWeb2=!1,_ExportReportingManager=class _ExportReportingManager{constructor(){this.fileActivityCache=new Map,this.allowedFormats=["json","csv","html","markdown"],this.activityTrackingDays=30,this.activityCutoffMs=null,this.timeTrackingIntegration="none",this._loadConfiguration(),this._setupConfigurationWatcher(),this.initialize()}_loadConfiguration(){try{let config=vscode3.workspace.getConfiguration("explorerDates"),configuredFormats=config.get("reportFormats",["json","html"]),defaults=["json","csv","html","markdown"];this.allowedFormats=Array.from(new Set([...configuredFormats,...defaults]));let days=config.get("activityTrackingDays",30);this.activityTrackingDays=Math.max(1,Math.min(365,days)),this.activityCutoffMs=this.activityTrackingDays*24*60*60*1e3,this.timeTrackingIntegration=config.get("timeTrackingIntegration","none")}catch(error){logger2.error("Failed to load reporting configuration",error)}}_setupConfigurationWatcher(){vscode3.workspace.onDidChangeConfiguration(event=>{(event.affectsConfiguration("explorerDates.reportFormats")||event.affectsConfiguration("explorerDates.activityTrackingDays")||event.affectsConfiguration("explorerDates.timeTrackingIntegration"))&&(this._loadConfiguration(),logger2.info("Reporting configuration updated",{allowedFormats:this.allowedFormats,activityTrackingDays:this.activityTrackingDays,timeTrackingIntegration:this.timeTrackingIntegration}))})}async initialize(){try{this.startFileWatcher(),logger2.info("Export & Reporting Manager initialized")}catch(error){logger2.error("Failed to initialize Export & Reporting Manager:",error)}}startFileWatcher(){let watcher=vscode3.workspace.createFileSystemWatcher("**/*");watcher.onDidChange(uri=>{this.recordFileActivity(uri,"modified")}),watcher.onDidCreate(uri=>{this.recordFileActivity(uri,"created")}),watcher.onDidDelete(uri=>{this.recordFileActivity(uri,"deleted")})}recordFileActivity(uri,action){try{let filePath=uri.fsPath||uri.path,timestamp=new Date;this.fileActivityCache.has(filePath)||this.fileActivityCache.set(filePath,[]),this.fileActivityCache.get(filePath).push({action,timestamp,path:filePath}),this._enforceActivityRetention(filePath)}catch(error){logger2.error("Failed to record file activity:",error)}}_enforceActivityRetention(filePath){let activities=this.fileActivityCache.get(filePath);if(!(!activities||activities.length===0)){if(this.activityCutoffMs){let cutoff=new Date(Date.now()-this.activityCutoffMs);for(;activities.length&&activities[0].timestamp<cutoff;)activities.shift()}activities.length>100&&activities.splice(0,activities.length-100)}}async generateFileModificationReport(options={}){try{let{format="json",timeRange="all",includeDeleted=!1,outputPath=null}=options;if(!this.allowedFormats.includes(format)){let warning=`Report format "${format}" is disabled. Allowed formats: ${this.allowedFormats.join(", ")}`;return vscode3.window.showWarningMessage(warning),logger2.warn(warning),null}let report=await this.collectFileData(timeRange,includeDeleted),formattedReport=await this.formatReport(report,format);return outputPath&&(await this.saveReport(formattedReport,outputPath),vscode3.window.showInformationMessage(`Report saved to ${outputPath}`)),formattedReport}catch(error){return logger2.error("Failed to generate file modification report:",error),vscode3.window.showErrorMessage("Failed to generate report"),null}}async collectFileData(timeRange,includeDeleted){let files=[],workspaceFolders=vscode3.workspace.workspaceFolders;if(!workspaceFolders)return{files:[],summary:this.createSummary([])};for(let folder of workspaceFolders){let folderFiles=await this.scanWorkspaceFolder(folder.uri,timeRange,includeDeleted);files.push(...folderFiles)}let summary=this.createSummary(files);return summary.integrationTarget=this.timeTrackingIntegration,summary.activityTrackingDays=this.activityTrackingDays,{generatedAt:new Date().toISOString(),workspace:workspaceFolders.map(f=>f.uri.fsPath),timeRange,files,summary}}async scanWorkspaceFolder(folderUri,timeRange,includeDeleted){let files=[],excludePatterns=vscode3.workspace.getConfiguration("explorerDates").get("excludedPatterns",[]);try{let entries=await vscode3.workspace.fs.readDirectory(folderUri);for(let[name,type]of entries){let fileUri=vscode3.Uri.joinPath(folderUri,name),relativePath=vscode3.workspace.asRelativePath(fileUri);if(!this.isExcluded(relativePath,excludePatterns)){if(type===vscode3.FileType.File){let fileData=await this.getFileData(fileUri,timeRange);fileData&&files.push(fileData)}else if(type===vscode3.FileType.Directory){let subFiles=await this.scanWorkspaceFolder(fileUri,timeRange,includeDeleted);files.push(...subFiles)}}}if(includeDeleted&&folderUri.fsPath){let deletedFiles=this.getDeletedFiles(folderUri.fsPath,timeRange);files.push(...deletedFiles)}}catch(error){logger2.error(`Failed to scan folder ${folderUri.fsPath||folderUri.path}:`,error)}return files}async getFileData(uri,timeRange){try{let stat=await vscode3.workspace.fs.stat(uri),relativePath=vscode3.workspace.asRelativePath(uri),cacheKey=uri.fsPath||uri.path,activities=this.fileActivityCache.get(cacheKey)||[],filteredActivities=this.filterActivitiesByTimeRange(activities,timeRange);return{path:relativePath,fullPath:cacheKey,size:stat.size,created:new Date(stat.ctime),modified:new Date(stat.mtime),type:this.getFileType(relativePath),extension:getExtension(relativePath),activities:filteredActivities,activityCount:filteredActivities.length,lastActivity:filteredActivities.length>0?filteredActivities[filteredActivities.length-1].timestamp:new Date(stat.mtime)}}catch(error){return logger2.error(`Failed to get file data for ${uri.fsPath||uri.path}:`,error),null}}filterActivitiesByTimeRange(activities,timeRange){let filtered=activities;if(timeRange!=="all"){let now=new Date,cutoff;switch(timeRange){case"24h":cutoff=new Date(now-1440*60*1e3);break;case"7d":cutoff=new Date(now-10080*60*1e3);break;case"30d":cutoff=new Date(now-720*60*60*1e3);break;case"90d":cutoff=new Date(now-2160*60*60*1e3);break;default:cutoff=null}cutoff&&(filtered=filtered.filter(activity=>activity.timestamp>=cutoff))}if(this.activityCutoffMs){let retentionCutoff=new Date(Date.now()-this.activityCutoffMs);filtered=filtered.filter(activity=>activity.timestamp>=retentionCutoff)}return filtered}getDeletedFiles(folderPath,timeRange){if(!folderPath)return[];let deletedFiles=[];for(let[filePath,activities]of this.fileActivityCache)if(filePath.startsWith(folderPath)){let deleteActivities=activities.filter(a=>a.action==="deleted"),filteredDeletes=this.filterActivitiesByTimeRange(deleteActivities,timeRange);filteredDeletes.length>0&&deletedFiles.push({path:vscode3.workspace.asRelativePath(filePath),fullPath:filePath,size:0,created:null,modified:null,type:"deleted",extension:getExtension(filePath),activities:filteredDeletes,activityCount:filteredDeletes.length,lastActivity:filteredDeletes[filteredDeletes.length-1].timestamp})}return deletedFiles}createSummary(files){let summary={totalFiles:files.length,totalSize:files.reduce((sum,file)=>sum+(file.size||0),0),fileTypes:{},activityByDay:{},mostActiveFiles:[],recentlyModified:[],largestFiles:[],oldestFiles:[]};files.forEach(file=>{let type=file.type||"unknown";summary.fileTypes[type]=(summary.fileTypes[type]||0)+1});let retentionWindow=new Date(Date.now()-this.activityTrackingDays*24*60*60*1e3);return files.forEach(file=>{file.activities.forEach(activity=>{if(activity.timestamp>=retentionWindow){let day=activity.timestamp.toISOString().split("T")[0];summary.activityByDay[day]=(summary.activityByDay[day]||0)+1}})}),summary.mostActiveFiles=files.sort((a,b)=>b.activityCount-a.activityCount).slice(0,10).map(file=>({path:file.path,activityCount:file.activityCount,lastActivity:file.lastActivity})),summary.recentlyModified=files.filter(file=>file.modified).sort((a,b)=>b.modified-a.modified).slice(0,20).map(file=>({path:file.path,modified:file.modified,size:file.size})),summary.largestFiles=files.sort((a,b)=>(b.size||0)-(a.size||0)).slice(0,10).map(file=>({path:file.path,size:file.size,modified:file.modified})),summary.oldestFiles=files.filter(file=>file.modified).sort((a,b)=>a.modified-b.modified).slice(0,10).map(file=>({path:file.path,modified:file.modified,size:file.size})),summary}async formatReport(report,format){switch(format.toLowerCase()){case"json":return JSON.stringify(report,null,2);case"csv":return this.formatAsCSV(report);case"html":return this.formatAsHTML(report);case"markdown":return this.formatAsMarkdown(report);default:throw new Error(`Unsupported format: ${format}`)}}formatAsCSV(report){let lines=["Path,Size,Created,Modified,Type,Extension,ActivityCount,LastActivity"];return report.files.forEach(file=>{lines.push([file.path,file.size||0,file.created?file.created.toISOString():"",file.modified?file.modified.toISOString():"",file.type,file.extension,file.activityCount,file.lastActivity?file.lastActivity.toISOString():""].join(","))}),lines.join(`
...
explorerDatesApi.registerPlugin('myPlugin', myPlugin);
    ...
    `}__name(generateWorkspaceActivityHTML,"generateWorkspaceActivityHTML");function generateDiagnosticsHTML(diagnostics){return`
        ...
        <body>
            ...
            ${Object.entries(diagnostics).map(([title,data])=>{let rows=Object.entries(data).map(([key,value])=>{let displayValue=Array.isArray(value)?value.join(", ")||"None":(value==null?void 0:value.toString())||"N/A";return`
                <tr>
