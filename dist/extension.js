...
`;let v=i.fileSizeLabel||this._formatFileSize(o.size,n||"auto");if(h+=`\u{1F4CA} Size: ${v} (${o.size.toLocaleString()} bytes)
...
`)}`;vscode.window.showInformationMessage(n,{modal:!0}),t.info("Current configuration displayed",s)}catch(r){t.error("Failed to show configuration",r)}})),i.push(vscode.commands.registerCommand("explorerDates.resetToDefaults",async()=>{try{let r=vscode.workspace.getConfiguration("explorerDates");await r.update("highContrastMode",!1,vscode.ConfigurationTarget.Global),await r.update("badgePriority","time",vscode.ConfigurationTarget.Global),await r.update("accessibilityMode",!1,vscode.ConfigurationTarget.Global),vscode.window.showInformationMessage("Reset high contrast, badge priority, and accessibility mode to defaults. Changes should take effect immediately."),t.info("Reset problematic settings to defaults"),e&&(e.clearAllCaches(),e.refreshAll())}catch(r){t.error("Failed to reset settings",r),vscode.window.showErrorMessage(`Failed to reset settings: ${r.message}`)}})),i.push(vscode.commands.registerCommand("explorerDates.toggleDecorations",()=>{try{let r=vscode.workspace.getConfiguration("explorerDates"),s=r.get("showDateDecorations",!0);r.update("showDateDecorations",!s,vscode.ConfigurationTarget.Global);let n=s?(o==null?void 0:o.getString("decorationsDisabled"))||"Date decorations disabled":(o==null?void 0:o.getString("decorationsEnabled"))||"Date decorations enabled";vscode.window.showInformationMessage(n),t.info(`Date decorations toggled to: ${!s}`)}catch(r){t.error("Failed to toggle decorations",r),vscode.window.showErrorMessage(`Failed to toggle decorations: ${r.message}`)}})),i.push(vscode.commands.registerCommand("explorerDates.copyFileDate",async r=>{try{let s=r;if(!s&&vscode.window.activeTextEditor&&(s=vscode.window.activeTextEditor.document.uri),!s){vscode.window.showWarningMessage("No file selected");return}let n=await fileSystem.stat(s),c=(n.mtime instanceof Date?n.mtime:new Date(n.mtime)).toLocaleString();await vscode.env.clipboard.writeText(c),vscode.window.showInformationMessage(`Copied to clipboard: ${c}`),t.info(`File date copied for: ${s.fsPath||s.path}`)}catch(s){t.error("Failed to copy file date",s),vscode.window.showErrorMessage(`Failed to copy file date: ${s.message}`)}})),i.push(vscode.commands.registerCommand("explorerDates.showFileDetails",async r=>{try{let s=r;if(!s&&vscode.window.activeTextEditor&&(s=vscode.window.activeTextEditor.document.uri),!s){vscode.window.showWarningMessage("No file selected");return}let n=await fileSystem.stat(s),c=getFileName(s.fsPath||s.path),l=(e==null?void 0:e._formatFileSize(n.size,"auto"))||`${n.size} bytes`,u=(n.mtime instanceof Date?n.mtime:new Date(n.mtime)).toLocaleString(),h=(n.birthtime instanceof Date?n.birthtime:new Date(n.birthtime||n.mtime)).toLocaleString(),v=`File: ${c}
Size: ${l}
Modified: ${u}
Created: ${h}
Path: ${s.fsPath||s.path}`;vscode.window.showInformationMessage(v,{modal:!0}),t.info(`File details shown for: ${s.fsPath||s.path}`)}catch(s){t.error("Failed to show file details",s),vscode.window.showErrorMessage(`Failed to show file details: ${s.message}`)}})),i.push(vscode.commands.registerCommand("explorerDates.toggleFadeOldFiles",()=>{try{let r=vscode.workspace.getConfiguration("explorerDates"),s=r.get("fadeOldFiles",!1);r.update("fadeOldFiles",!s,vscode.ConfigurationTarget.Global);let n=s?"Fade old files disabled":"Fade old files enabled";vscode.window.showInformationMessage(n),t.info(`Fade old files toggled to: ${!s}`)}catch(r){t.error("Failed to toggle fade old files",r),vscode.window.showErrorMessage(`Failed to toggle fade old files: ${r.message}`)}})),i.push(vscode.commands.registerCommand("explorerDates.showFileHistory",async r=>{try{if(isWeb){vscode.window.showInformationMessage("Git history is unavailable on VS Code for Web.");return}let s=r;if(!s&&vscode.window.activeTextEditor&&(s=vscode.window.activeTextEditor.document.uri),!s){vscode.window.showWarningMessage("No file selected");return}let n=vscode.workspace.getWorkspaceFolder(s);if(!n){vscode.window.showWarningMessage("File is not in a workspace");return}let l=`git log --oneline -10 -- "${getRelativePath(n.uri.fsPath||n.uri.path,s.fsPath||s.path)}"`;loadChildProcess().exec(l,{cwd:n.uri.fsPath,timeout:3e3},(h,v)=>{if(h){h.message.includes("not a git repository")?vscode.window.showWarningMessage("This file is not in a Git repository"):vscode.window.showErrorMessage(`Git error: ${h.message}`);return}if(!v.trim()){vscode.window.showInformationMessage("No Git history found for this file");return}let g=v.trim(),w=getFileName(s.fsPath||s.path);vscode.window.showInformationMessage(`Recent commits for ${w}:
...
explorerDatesApi.registerPlugin('myPlugin', myPlugin);
    ...
    `}d(xi,"generateWorkspaceActivityHTML");function Di(a){return`
        ...
        <body>
            ...
            ${Object.entries(a).map(([t,o])=>{let i=Object.entries(o).map(([r,s])=>{let n=Array.isArray(s)?s.join(", ")||"None":(s==null?void 0:s.toString())||"N/A";return`
                <tr>
    ...
    `}d(Fi,"generatePerformanceAnalyticsHTML");function to(a){let e=b.window.createStatusBarItem(b.StatusBarAlignment.Right,100);e.command="explorerDates.showFileDetails",e.tooltip="Click to show detailed file information";let t=d(async()=>{try{let o=b.window.activeTextEditor;if(!o){e.hide();return}let i=o.document.uri;if(i.scheme!=="file"){e.hide();return}let r=await vi.stat(i),s=r.mtime instanceof Date?r.mtime:new Date(r.mtime),n=O._formatDateBadge(s,"smart"),c=O._formatFileSize(r.size,"auto");e.text=`$(clock) ${n} $(file) ${c}`,e.show()}catch(o){e.hide(),D.debug("Failed to update status bar",o)}},"updateStatusBar");return b.window.onDidChangeActiveTextEditor(t),b.window.onDidChangeTextEditorSelection(t),t(),a.subscriptions.push(e),e}d(to,"initializeStatusBar");async function Ti(a){try{D=mi(),de=wi(),a.subscriptions.push(de),D.info("Explorer Dates: Extension activated");let e=b.env.uiKind===b.UIKind.Web;await b.commands.executeCommand("setContext","explorerDates.gitFeaturesAvailable",!e);let t=b.workspace.getConfiguration("explorerDates"),o=t.get("enableWorkspaceTemplates",!0),i=t.get("enableReporting",!0),r=t.get("enableExtensionApi",!0);O=new fi;let s=b.window.registerFileDecorationProvider(O);a.subscriptions.push(s),a.subscriptions.push(O),a.subscriptions.push(D),await O.initializeAdvancedSystems(a);let n=null,c=null,l=null,u=null,h=d(()=>{if(!n){let{OnboardingManager:_}=Kt();n=new _(a)}return n},"getOnboardingManager"),v=d(()=>{if(!o)throw new Error("Workspace templates are disabled via explorerDates.enableWorkspaceTemplates");if(!c){let{WorkspaceTemplatesManager:_}=Yt();c=new _(a)}return c},"getWorkspaceTemplatesManager"),g=d(()=>{if(!l){let{ExtensionApiManager:_}=Xt();l=new _,a.subscriptions.push(l)}return l},"getExtensionApiManager"),w=d(()=>{if(!i)throw new Error("Reporting is disabled via explorerDates.enableReporting");if(!u){let{ExportReportingManager:_}=eo();u=new _,a.subscriptions.push(u)}return u},"getExportReportingManager"),U=d(()=>g().getApi(),"apiFactory");r?a.exports=U:(a.exports=void 0,D.info("Explorer Dates API exports disabled via explorerDates.enableExtensionApi")),b.workspace.getConfiguration("explorerDates").get("showWelcomeOnStartup",!0)&&await h().shouldShowOnboarding()&&setTimeout(()=>{h().showWelcomeMessage()},5e3),bi({context:a,fileDateProvider:O,logger:D,l10n:de}),_i({context:a,fileDateProvider:O,logger:D,generators:{generateWorkspaceActivityHTML:xi,generatePerformanceAnalyticsHTML:Fi,generateDiagnosticsHTML:Di,generateDiagnosticsWebview:Si}}),yi({context:a,logger:D,getOnboardingManager:h});let be=b.commands.registerCommand("explorerDates.openTemplateManager",async()=>{try{if(!o){b.window.showInformationMessage("Workspace templates are disabled. Enable explorerDates.enableWorkspaceTemplates to use this command.");return}await v().showTemplateManager(),D.info("Template manager opened")}catch(_){D.error("Failed to open template manager",_),b.window.showErrorMessage(`Failed to open template manager: ${_.message}`)}});a.subscriptions.push(be);let he=b.commands.registerCommand("explorerDates.saveTemplate",async()=>{try{if(!o){b.window.showInformationMessage("Workspace templates are disabled. Enable explorerDates.enableWorkspaceTemplates to save templates.");return}let _=await b.window.showInputBox({prompt:"Enter template name",placeHolder:"e.g., My Project Setup"});if(_){let X=await b.window.showInputBox({prompt:"Enter description (optional)",placeHolder:"Brief description of this template"})||"";await v().saveCurrentConfiguration(_,X)}D.info("Template saved")}catch(_){D.error("Failed to save template",_),b.window.showErrorMessage(`Failed to save template: ${_.message}`)}});a.subscriptions.push(he);let te=b.commands.registerCommand("explorerDates.generateReport",async()=>{try{if(!i){b.window.showInformationMessage("Reporting features are disabled. Enable explorerDates.enableReporting to generate reports.");return}await w().showReportDialog(),D.info("Report generation started")}catch(_){D.error("Failed to generate report",_),b.window.showErrorMessage(`Failed to generate report: ${_.message}`)}});a.subscriptions.push(te);let _e=b.commands.registerCommand("explorerDates.showApiInfo",async()=>{try{if(!r){b.window.showInformationMessage("Explorer Dates API is disabled via settings.");return}let _=b.window.createWebviewPanel("apiInfo","Explorer Dates API Information",b.ViewColumn.One,{enableScripts:!0});_.webview.html=Ci(U()),D.info("API information panel opened")}catch(_){D.error("Failed to show API information",_),b.window.showErrorMessage(`Failed to show API information: ${_.message}`)}});a.subscriptions.push(_e);let P,oe=b.workspace.getConfiguration("explorerDates"),ue=oe.get("performanceMode",!1);oe.get("showStatusBar",!1)&&!ue&&(P=to(a));let ie=b.workspace.onDidChangeConfiguration(_=>{if(_.affectsConfiguration("explorerDates.showStatusBar")||_.affectsConfiguration("explorerDates.performanceMode")){let X=b.workspace.getConfiguration("explorerDates"),re=X.get("showStatusBar",!1),q=X.get("performanceMode",!1),Ce=re&&!q;Ce&&!P?P=to(a):!Ce&&P&&(P.dispose(),P=null)}});a.subscriptions.push(ie),D.info("Explorer Dates: Date decorations ready")}catch(e){let t=`${de?de.getString("activationError"):"Explorer Dates failed to activate"}: ${e.message}`;throw D&&D.error("Extension activation failed",e),b.window.showErrorMessage(t),e}}d(Ti,"activate");async function ki(){try{D&&D.info("Explorer Dates extension is being deactivated"),O&&typeof O.dispose=="function"&&await O.dispose(),D&&D.info("Explorer Dates extension deactivated successfully")}catch(a){D&&D.error("Explorer Dates: Error during deactivation",a)}}d(ki,"deactivate");module.exports={activate:Ti,deactivate:ki};
