...
`;let w=o.fileSizeLabel||this._formatFileSize(i.size,n||"auto");if(h+=`\u{1F4CA} Size: ${w} (${i.size.toLocaleString()} bytes)
`,u&&(h+=`\u{1F3F7}\uFE0F Type: ${u.toUpperCase()} file
`),c)try{let g=t||e,O=(await this._fileSystem.readFile(g,"utf8")).split(`
...
\u{1F464} Last Modified By: ${s.authorName}`,s.authorEmail&&(h+=` (${s.authorEmail})`),s.authorDate&&(h+=`
   ${s.authorDate}`)),h}_formatDateReadable(e){let t=new Date,i=t.getTime()-e.getTime(),o=Math.floor(i/(1e3*60)),s=Math.floor(i/(1e3*60*60)),r=Math.floor(i/(1e3*60*60*24)),n=this._buildReadableDescriptor(e,t,o,s,r);return n?this._getFlyweightValue(this._readableDateFlyweightCache,this._readableDateFlyweightOrder,this._readableDateFlyweightLimit,n.key,n.factory,this._readableFlyweightStats):e.getFullYear()===t.getFullYear()?this._l10n.formatDate(e,{month:"short",day:"numeric"}):this._l10n.formatDate(e,{month:"short",day:"numeric",year:"numeric"})}async _getGitBlameInfo(e,t=null){if(!this._gitAvailable||!ne)return null;try{let i=m.workspace.getWorkspaceFolder(m.Uri.file(e));if(!i)return null;let o=i.uri.fsPath||i.uri.path,s=Hi(o,e),r=this._getGitCacheKey(o,s,t),n=this._getCachedGitInfo(r);if(n)return n;let c=Date.now();try{let{stdout:l}=await ne(`git log -1 --format="%H|%an|%ae|%ad" -- "${s}"`,{cwd:i.uri.fsPath,timeout:2e3});if(!l||!l.trim())return null;let[u,h,w,g]=l.trim().split("|"),_={hash:u||"",authorName:h||"Unknown",authorEmail:w||"",authorDate:g||""};return this._setCachedGitInfo(r,_),_}finally{let l=Date.now()-c;this._metrics.gitBlameTimeMs+=l,this._metrics.gitBlameCalls++}}catch{return null}}_getInitials(e){if(!e||typeof e!="string")return null;let t=e.trim().split(/\s+/).filter(Boolean);return t.length===0?null:t.length===1?t[0].substring(0,2).toUpperCase():(t[0][0]+(t[1][0]||"")).substring(0,2).toUpperCase()}_formatCompactSize(e){if(typeof e!="number"||isNaN(e))return null;let t=["B","K","M","G","T"],i=0,o=e;for(;o>=1024&&i<t.length-1;)o=o/1024,i++;let s=Math.round(o),r=t[i];if(s<=9)return`${s}${r}`;let n=String(s);return n.length>=2?n.slice(0,2):n}_formatFullDate(e){let t={year:"numeric",month:"long",day:"numeric",hour:"2-digit",minute:"2-digit",second:"2-digit",timeZoneName:"short"};return e.toLocaleString("en-US",t)}_getCacheKey(e){let t=Gi(J(e));return this._cacheNamespace?`${this._cacheNamespace}${Ne}${t}`:t}_stripNamespaceFromCacheKey(e){if(!e||typeof e!="string")return e;let t=e.indexOf(Ne);return t===-1?e:e.slice(t+Ne.length)}_stableHash(e){let t=typeof e=="string"?e:JSON.stringify(e),i=0;for(let o=0;o<t.length;o++)i=(i<<5)-i+t.charCodeAt(o),i|=0;return Math.abs(i).toString(16)}_buildCacheNamespace(e){try{let t=e||m.workspace.getConfiguration("explorerDates"),i={dateDecorationFormat:t.get("dateDecorationFormat","smart"),showGitInfo:t.get("showGitInfo",!0),showFileSize:t.get("showFileSize",!1),fileSizeFormat:t.get("fileSizeFormat","auto"),colorScheme:t.get("colorScheme","recency"),highContrastMode:t.get("highContrastMode",!1),fadeOldFiles:t.get("fadeOldFiles",!1),fadeThreshold:t.get("fadeThreshold",72),badgePriority:t.get("badgePriority","time"),showDateDecorations:t.get("showDateDecorations",!0),excludedFolders:(t.get("excludedFolders",[])||[]).join("|"),excludedPatterns:(t.get("excludedPatterns",[])||[]).join("|"),customColors:JSON.stringify(t.get("customColors",{})),metadataVersion:2};return`ns-${this._stableHash(i).slice(0,8)}`}catch(t){return this._logger.debug(`Failed to build cache namespace: ${t.message}`),"ns-default"}}_updateCacheNamespace(e){let t=this._buildCacheNamespace(e);if(t&&t!==this._cacheNamespace){let i=this._cacheNamespace;return this._cacheNamespace=t,this._logger.info(`Cache namespace updated (${i||"unset"} \u2192 ${t})`),!0}return!1}_isFileNotFoundError(e){return e?e.code==="ENOENT"?!0:typeof e.message=="string"&&e.message.includes("ENOENT"):!1}async provideFileDecoration(e,t){let i=Date.now();try{if(!e){this._logger.error("\u274C Invalid URI provided to provideFileDecoration:",e);return}let o=J(e);if(!o){this._logger.error("\u274C Could not resolve path for URI in provideFileDecoration:",e);return}let s=q(o),r=e.scheme||"file";if(r!=="file"){this._logger.debug(`\u23ED\uFE0F Skipping decoration for ${s} (unsupported scheme: ${r})`);return}this._performanceMode||(this._logger.debug(`\u{1F50D} VSCODE REQUESTED DECORATION: ${s} (${o})`),this._logger.infoWithOptions(this._stressLogOptions,`\u{1F4CA} Call context: token=${!!t}, cancelled=${t?.isCancellationRequested}`));let n=m.workspace.getConfiguration("explorerDates"),c=d(($,rt)=>{if(this._previewSettings&&Object.prototype.hasOwnProperty.call(this._previewSettings,$)){let at=this._previewSettings[$];return this._logger.debug(`\u{1F3AD} Using preview value for ${$}: ${at} (config has: ${n.get($,rt)})`),at}return n.get($,rt)},"_get");if(this._previewSettings&&this._logger.infoWithOptions(this._stressLogOptions,`\u{1F3AD} Processing ${s} in PREVIEW MODE with settings:`,()=>this._previewSettings),!c("showDateDecorations",!0)){this._performanceMode||this._logger.infoWithOptions(this._stressLogOptions,`\u274C RETURNED UNDEFINED: Decorations disabled globally for ${s}`);return}if(await this._isExcludedSimple(e)){this._performanceMode||this._logger.infoWithOptions(this._stressLogOptions,`\u274C File excluded: ${s}`);return}this._logger.debug(`\u{1F50D} Processing file: ${s}`);let l=this._getCacheKey(e);if(this._previewSettings)this._logger.debug(`\u{1F504} Skipping cache due to active preview settings for: ${s}`);else{let $=await this._getCachedDecoration(l,s);if($)return $}if(this._metrics.cacheMisses++,this._logger.debug(`\u274C Cache miss for: ${s} (key: ${l.substring(0,50)}...)`),t?.isCancellationRequested){this._logger.debug(`Decoration cancelled for: ${o}`);return}let u=Date.now(),h;try{h=await this._fileSystem.stat(e)}catch($){if(this._metrics.fileStatTimeMs+=Date.now()-u,this._metrics.fileStatCalls++,this._isFileNotFoundError($)){this._logger.debug(`\u23ED\uFE0F Skipping decoration for ${s}: file not found (${$.message||$})`);return}throw $}if(this._metrics.fileStatTimeMs+=Date.now()-u,this._metrics.fileStatCalls++,!(typeof h.isFile=="function"?h.isFile():!0))return;let g=h.mtime instanceof Date?h.mtime:new Date(h.mtime),_=h.birthtime instanceof Date?h.birthtime:new Date(h.birthtime||h.ctime||h.mtime),O={mtime:g,birthtime:_,size:h.size},B=Date.now()-g.getTime(),ve=c("dateDecorationFormat","smart"),U=this._performanceMode?"none":c("colorScheme","none"),le=c("highContrastMode",!1),de=this._performanceMode?!1:c("showFileSize",!1),j=c("fileSizeFormat","auto"),Q=c("accessibilityMode",!1),_e=this._performanceMode?!1:c("fadeOldFiles",!1),ot=c("fadeThreshold",30),Z=this._performanceMode?"none":c("showGitInfo","none"),v=this._performanceMode?"time":c("badgePriority","time");this._lightweightMode&&(U="none",de=!1,Q=!1,Z="none",v="time");let te=(Z!=="none"||v==="author")&&this._gitAvailable&&!this._performanceMode,he=te?Z:"none";v==="author"&&!te&&(v="time");let ie=te?await this._getGitBlameInfo(o,g.getTime()):null,be=this._generateBadgeDetails({filePath:o,stat:O,diffMs:B,dateFormat:ve,badgePriority:v,showFileSize:de,fileSizeFormat:j,gitBlame:ie,showGitInfo:he}),oi=me(o),si=[".js",".ts",".jsx",".tsx",".py",".rb",".php",".java",".cpp",".c",".cs",".go",".rs",".kt",".swift"].includes(oi),st=Q&&this._accessibility?.shouldEnhanceAccessibility();this._logger.debug(`\u{1F50D} Tooltip generation for ${s}: accessibilityMode=${Q}, shouldUseAccessible=${st}, previewMode=${!!this._previewSettings}`);let ye=await this._buildTooltipContent({filePath:o,resourceUri:e,stat:O,badgeDetails:be,gitBlame:he==="none"?null:ie,shouldUseAccessibleTooltips:st,fileSizeFormat:j,isCodeFile:si}),oe;U!=="none"&&(oe=this._themeIntegration?this._themeIntegration.applyThemeAwareColorScheme(U,o,B):this._getColorByScheme(g,U,o)),this._logger.debug(`\u{1F3A8} Color scheme setting: ${U}, using color: ${oe?"yes":"no"}`);let Ce=Mt(be.displayBadge)||Mt(be.badge)||"??";this._accessibility?.shouldEnhanceAccessibility()&&(Ce=this._accessibility.getAccessibleBadge(Ce));let z,G=oe;G&&(G=this._enhanceColorForSelection(G),this._logger.debug(`\u{1F3A8} Added enhanced color: ${G.id||G} (original: ${oe?.id||oe})`)),_e&&Math.floor(B/864e5)>ot&&(G=new m.ThemeColor("editorGutter.commentRangeForeground")),le&&(G=new m.ThemeColor("editorWarning.foreground"),this._logger.info(`\u{1F506} Applied high contrast color (overriding colorScheme=${U})`));let xe=ye&&ye.length<500?ye:void 0;try{z=this._acquireDecorationFromPool({badge:Ce,tooltip:xe,color:G}),xe&&this._logger.debug(`\u{1F4DD} Added tooltip (${xe.length} chars)`)}catch($){this._logger.error("\u274C Failed to create decoration:",$),z=new m.FileDecoration("!!"),z.propagate=!1}if(this._logger.debug(`\u{1F3A8} Color/contrast check for ${s}: colorScheme=${U}, highContrastMode=${le}, hasColor=${!!G}, previewMode=${!!this._previewSettings}`),this._previewSettings?this._logger.debug(`\u{1F504} Skipping cache storage due to preview mode for: ${s}`):await this._storeDecorationInCache(l,z,s,e),this._metrics.totalDecorations++,this._maybeShedWorkload(),this._maybePurgeLightweightCaches(),!z?.badge){this._logger.error(`\u274C Decoration badge is invalid for: ${s}`);return}let ri=Date.now()-i;return this._performanceMode||(this._logger.infoWithOptions(this._stressLogOptions,`\u2705 Decoration created for: ${s} (badge: ${z.badge||"undefined"}) - Cache key: ${l.substring(0,30)}...`),this._logger.infoWithOptions(this._stressLogOptions,"\u{1F3AF} RETURNING DECORATION TO VSCODE:",()=>({file:s,badge:z.badge,hasTooltip:!!z.tooltip,hasColor:!!z.color,colorType:z.color?.constructor?.name,processingTimeMs:ri,decorationType:z.constructor.name}))),z}catch(o){this._metrics.errors++;let s=i?Date.now()-i:0,r=q(e),n=J(e)||"unknown-uri";this._logger.error(`\u274C DECORATION ERROR for ${r}:`,{error:o.message,stack:o.stack?.split(`
`)[0],processingTimeMs:s,uri:n}),this._logger.error(`\u274C CRITICAL ERROR DETAILS for ${r}: ${o.message}`),this._logger.error(`\u274C Error type: ${o.constructor.name}`),this._logger.error(`\u274C Full stack: ${o.stack}`),this._logger.info(`\u274C RETURNED UNDEFINED: Error occurred for ${r}`);return}}getMetrics(){let e={...this._metrics,cacheSize:this._decorationCache.size,cacheHitRate:this._metrics.cacheHits+this._metrics.cacheMisses>0?(this._metrics.cacheHits/(this._metrics.cacheHits+this._metrics.cacheMisses)*100).toFixed(2)+"%":"0.00%",forceCacheBypass:this._forceCacheBypass,decorationPoolEnabled:this._enableDecorationPool,flyweightsEnabled:this._enableFlyweights,lightweightMode:this._lightweightMode,memorySheddingEnabled:this._memorySheddingEnabled,memorySheddingActive:this._memorySheddingActive};e.decorationPool={size:this._decorationPool.size,hits:this._decorationPoolStats.hits,misses:this._decorationPoolStats.misses},e.badgeFlyweight={...this._badgeFlyweightStats,cacheSize:this._badgeFlyweightCache.size},e.readableFlyweight={...this._readableFlyweightStats,cacheSize:this._readableDateFlyweightCache.size},this._advancedCache&&(e.advancedCache=this._advancedCache.getStats()),this._batchProcessor&&(e.batchProcessor=this._batchProcessor.getMetrics());let t=Array.from(this._decorationCache.keys()).slice(0,5).map(i=>this._stripNamespaceFromCacheKey(i));return e.cacheDebugging={memoryCacheKeys:t,cacheTimeout:this._cacheTimeout,maxCacheSize:this._maxCacheSize,keyStatsSize:this._cacheKeyStats?this._cacheKeyStats.size:0,cacheNamespace:this._cacheNamespace},e.performanceTiming={avgGitBlameMs:this._metrics.gitBlameCalls>0?(this._metrics.gitBlameTimeMs/this._metrics.gitBlameCalls).toFixed(1):"0.0",avgFileStatMs:this._metrics.fileStatCalls>0?(this._metrics.fileStatTimeMs/this._metrics.fileStatCalls).toFixed(1):"0.0",totalGitBlameTimeMs:this._metrics.gitBlameTimeMs,totalFileStatTimeMs:this._metrics.fileStatTimeMs,gitBlameCalls:this._metrics.gitBlameCalls,fileStatCalls:this._metrics.fileStatCalls},e}async initializeAdvancedSystems(e){try{if(this._extensionContext=e,this._isWeb&&await this._maybeWarnAboutGitLimitations(),this._performanceMode){this._logger.info("Performance mode enabled - skipping advanced cache, batch processor, and progressive loading");return}this._advancedCache=new Ni(e),await this._advancedCache.initialize(),this._logger.info("Advanced cache initialized"),this._batchProcessor.initialize(),this._logger.info("Batch processor initialized"),await this._applyProgressiveLoadingSetting(),m.workspace.getConfiguration("explorerDates").get("autoThemeAdaptation",!0)&&(await this._themeIntegration.autoConfigureForTheme(),this._logger.info("Theme integration configured")),this._accessibility.shouldEnhanceAccessibility()&&(await this._accessibility.applyAccessibilityRecommendations(),this._logger.info("Accessibility recommendations applied"));try{await this._smartExclusion.cleanupAllWorkspaceProfiles()}catch(i){this._logger.warn("Failed to clean workspace exclusion profiles",i)}if(m.workspace.workspaceFolders)for(let i of m.workspace.workspaceFolders)try{await this._smartExclusion.suggestExclusions(i.uri),this._logger.info(`Smart exclusions analyzed for: ${i.name}`)}catch(o){this._logger.error(`Failed to analyze smart exclusions for ${i.name}`,o)}this._logger.info("Advanced systems initialized successfully")}catch(t){this._logger.error("Failed to initialize advanced systems",t)}}async _maybeWarnAboutGitLimitations(){if(!this._gitWarningShown){this._gitWarningShown=!0;try{let e=this._extensionContext?.globalState,t=Ki.WEB_GIT_NOTICE;if(e?.get(t,!1))return;if(e?.update)try{await e.update(t,!0)}catch(o){this._logger.debug("Failed to persist Git limitation notice flag",o)}Promise.resolve().then(()=>{m.window.showInformationMessage("Explorer Dates: Git attribution badges are unavailable on VS Code for Web. Time-based decorations remain available.")})}catch(e){this._logger.debug("Failed to display Git limitation notice",e)}}}_enhanceColorForSelection(e){let t={"charts.yellow":"list.warningForeground","charts.red":"list.errorForeground","charts.green":"list.highlightForeground","charts.blue":"symbolIcon.functionForeground","charts.purple":"symbolIcon.classForeground","charts.orange":"list.warningForeground","terminal.ansiYellow":"list.warningForeground","terminal.ansiGreen":"list.highlightForeground","terminal.ansiRed":"list.errorForeground","terminal.ansiBlue":"symbolIcon.functionForeground","terminal.ansiMagenta":"symbolIcon.classForeground","terminal.ansiCyan":"symbolIcon.stringForeground","editorGutter.commentRangeForeground":"list.deemphasizedForeground","editorWarning.foreground":"list.warningForeground","editorError.foreground":"list.errorForeground","editorInfo.foreground":"list.highlightForeground"},i=e.id||e,o=t[i];return o?(this._logger.debug(`\u{1F527} Enhanced color ${i} \u2192 ${o} for better selection visibility`),new m.ThemeColor(o)):e}async dispose(){this._logger.info("Disposing FileDateDecorationProvider",this.getMetrics()),this._refreshTimer&&(clearInterval(this._refreshTimer),this._refreshTimer=null,this._logger.info("Cleared periodic refresh timer")),this._cancelIncrementalRefreshTimers(),this._advancedCache&&await this._advancedCache.dispose(),this._cancelProgressiveWarmupJobs(),this._batchProcessor&&this._batchProcessor.dispose(),this._accessibility&&typeof this._accessibility.dispose=="function"&&this._accessibility.dispose(),this._decorationCache.clear(),this._clearDecorationPool("dispose"),this._gitCache.clear(),this._onDidChangeFileDecorations.dispose(),this._fileWatcher&&this._fileWatcher.dispose(),this._configurationWatcher&&(this._configurationWatcher.dispose(),this._configurationWatcher=null)}};d(je,"FileDateDecorationProvider");var Ue=je;It.exports={FileDateDecorationProvider:Ue}});var zt=S((exports,module)=>{var vscode=require("vscode"),{fileSystem}=H(),{getFileName,getRelativePath}=N(),isWeb=!0,childProcess=null;function loadChildProcess(){return!childProcess&&!isWeb&&(childProcess=eval("require")("child_process")),childProcess}d(loadChildProcess,"loadChildProcess");function registerCoreCommands({context:a,fileDateProvider:e,logger:t,l10n:i}){let o=[];o.push(vscode.commands.registerCommand("explorerDates.refreshDateDecorations",()=>{try{if(e){e.clearAllCaches(),e.refreshAll();let s=i?.getString("refreshSuccess")||"Date decorations refreshed - all caches cleared";vscode.window.showInformationMessage(s),t.info("Date decorations refreshed manually with cache clear")}}catch(s){t.error("Failed to refresh decorations",s),vscode.window.showErrorMessage(`Failed to refresh decorations: ${s.message}`)}})),o.push(vscode.commands.registerCommand("explorerDates.previewConfiguration",s=>{try{e&&(e.applyPreviewSettings(s),t.info("Configuration preview applied",s))}catch(r){t.error("Failed to apply configuration preview",r)}})),o.push(vscode.commands.registerCommand("explorerDates.clearPreview",()=>{try{e&&(e.applyPreviewSettings(null),t.info("Configuration preview cleared"))}catch(s){t.error("Failed to clear configuration preview",s)}})),o.push(vscode.commands.registerCommand("explorerDates.showMetrics",()=>{try{if(e){let s=e.getMetrics(),r=`Explorer Dates Metrics:
...
explorerDatesApi.registerPlugin('myPlugin', myPlugin);
    ...
    `}d(Do,"generateWorkspaceActivityHTML");function So(a){return`
        ...
        <body>
            ...
            ${Object.entries(a).map(([t,i])=>{let o=Object.entries(i).map(([s,r])=>{let n=Array.isArray(r)?r.join(", ")||"None":r?.toString()||"N/A";return`
                <tr>
    ...
    `}d(To,"generatePerformanceAnalyticsHTML");function Zt(a){let e=b.window.createStatusBarItem(b.StatusBarAlignment.Right,100);e.command="explorerDates.showFileDetails",e.tooltip="Click to show detailed file information";let t=d(async()=>{try{let i=b.window.activeTextEditor;if(!i){e.hide();return}let o=i.document.uri;if(o.scheme!=="file"){e.hide();return}let s=await _o.stat(o),r=s.mtime instanceof Date?s.mtime:new Date(s.mtime),n=W._formatDateBadge(r,"smart"),c=W._formatFileSize(s.size,"auto");e.text=`$(clock) ${n} $(file) ${c}`,e.show()}catch(i){e.hide(),D.debug("Failed to update status bar",i)}},"updateStatusBar");return b.window.onDidChangeActiveTextEditor(t),b.window.onDidChangeTextEditorSelection(t),t(),a.subscriptions.push(e),e}d(Zt,"initializeStatusBar");async function ko(a){try{D=wo(),ce=vo(),a.subscriptions.push(ce),D.info("Explorer Dates: Extension activated");let e=b.env.uiKind===b.UIKind.Web;await b.commands.executeCommand("setContext","explorerDates.gitFeaturesAvailable",!e);let t=b.workspace.getConfiguration("explorerDates"),i=t.get("enableWorkspaceTemplates",!0),o=t.get("enableReporting",!0),s=t.get("enableExtensionApi",!0);W=new fo;let r=b.window.registerFileDecorationProvider(W);a.subscriptions.push(r),a.subscriptions.push(W),a.subscriptions.push(D),await W.initializeAdvancedSystems(a);let n=null,c=null,l=null,u=null,h=d(()=>{if(!n){let{OnboardingManager:v}=qt();n=new v(a)}return n},"getOnboardingManager"),w=d(()=>{if(!i)throw new Error("Workspace templates are disabled via explorerDates.enableWorkspaceTemplates");if(!c){let{WorkspaceTemplatesManager:v}=Kt();c=new v(a)}return c},"getWorkspaceTemplatesManager"),g=d(()=>{if(!l){let{ExtensionApiManager:v}=Yt();l=new v,a.subscriptions.push(l)}return l},"getExtensionApiManager"),_=d(()=>{if(!o)throw new Error("Reporting is disabled via explorerDates.enableReporting");if(!u){let{ExportReportingManager:v}=Xt();u=new v,a.subscriptions.push(u)}return u},"getExportReportingManager"),O=d(()=>g().getApi(),"apiFactory");s?a.exports=O:(a.exports=void 0,D.info("Explorer Dates API exports disabled via explorerDates.enableExtensionApi")),b.workspace.getConfiguration("explorerDates").get("showWelcomeOnStartup",!0)&&await h().shouldShowOnboarding()&&setTimeout(()=>{h().showWelcomeMessage()},5e3),bo({context:a,fileDateProvider:W,logger:D,l10n:ce}),yo({context:a,fileDateProvider:W,logger:D,generators:{generateWorkspaceActivityHTML:Do,generatePerformanceAnalyticsHTML:To,generateDiagnosticsHTML:So,generateDiagnosticsWebview:Fo}}),Co({context:a,logger:D,getOnboardingManager:h});let ve=b.commands.registerCommand("explorerDates.openTemplateManager",async()=>{try{if(!i){b.window.showInformationMessage("Workspace templates are disabled. Enable explorerDates.enableWorkspaceTemplates to use this command.");return}await w().showTemplateManager(),D.info("Template manager opened")}catch(v){D.error("Failed to open template manager",v),b.window.showErrorMessage(`Failed to open template manager: ${v.message}`)}});a.subscriptions.push(ve);let U=b.commands.registerCommand("explorerDates.saveTemplate",async()=>{try{if(!i){b.window.showInformationMessage("Workspace templates are disabled. Enable explorerDates.enableWorkspaceTemplates to save templates.");return}let v=await b.window.showInputBox({prompt:"Enter template name",placeHolder:"e.g., My Project Setup"});if(v){let ee=await b.window.showInputBox({prompt:"Enter description (optional)",placeHolder:"Brief description of this template"})||"";await w().saveCurrentConfiguration(v,ee)}D.info("Template saved")}catch(v){D.error("Failed to save template",v),b.window.showErrorMessage(`Failed to save template: ${v.message}`)}});a.subscriptions.push(U);let le=b.commands.registerCommand("explorerDates.generateReport",async()=>{try{if(!o){b.window.showInformationMessage("Reporting features are disabled. Enable explorerDates.enableReporting to generate reports.");return}await _().showReportDialog(),D.info("Report generation started")}catch(v){D.error("Failed to generate report",v),b.window.showErrorMessage(`Failed to generate report: ${v.message}`)}});a.subscriptions.push(le);let de=b.commands.registerCommand("explorerDates.showApiInfo",async()=>{try{if(!s){b.window.showInformationMessage("Explorer Dates API is disabled via settings.");return}let v=b.window.createWebviewPanel("apiInfo","Explorer Dates API Information",b.ViewColumn.One,{enableScripts:!0});v.webview.html=xo(O()),D.info("API information panel opened")}catch(v){D.error("Failed to show API information",v),b.window.showErrorMessage(`Failed to show API information: ${v.message}`)}});a.subscriptions.push(de);let j,Q=b.workspace.getConfiguration("explorerDates"),_e=Q.get("performanceMode",!1);Q.get("showStatusBar",!1)&&!_e&&(j=Zt(a));let Z=b.workspace.onDidChangeConfiguration(v=>{if(v.affectsConfiguration("explorerDates.showStatusBar")||v.affectsConfiguration("explorerDates.performanceMode")){let ee=b.workspace.getConfiguration("explorerDates"),te=ee.get("showStatusBar",!1),he=ee.get("performanceMode",!1),ie=te&&!he;ie&&!j?j=Zt(a):!ie&&j&&(j.dispose(),j=null)}});a.subscriptions.push(Z),D.info("Explorer Dates: Date decorations ready")}catch(e){let t=`${ce?ce.getString("activationError"):"Explorer Dates failed to activate"}: ${e.message}`;throw D&&D.error("Extension activation failed",e),b.window.showErrorMessage(t),e}}d(ko,"activate");async function $o(){try{D&&D.info("Explorer Dates extension is being deactivated"),W&&typeof W.dispose=="function"&&await W.dispose(),D&&D.info("Explorer Dates extension deactivated successfully")}catch(a){D&&D.error("Explorer Dates: Error during deactivation",a)}}d($o,"deactivate");ei.exports={activate:ko,deactivate:$o}});var ii=ti();module.exports={activate:ii.activate,deactivate:ii.deactivate};
