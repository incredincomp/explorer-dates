...
explorerDatesApi.registerPlugin('myPlugin', myPlugin);
    ...
    `}d($i,"generatePerformanceAnalyticsHTML");function Xt(a){let e=y.window.createStatusBarItem(y.StatusBarAlignment.Right,100);e.command="explorerDates.showFileDetails",e.tooltip="Click to show detailed file information";let t=d(async()=>{try{let o=y.window.activeTextEditor;if(!o){e.hide();return}let i=o.document.uri;if(i.scheme!=="file"){e.hide();return}let s=await _i.stat(i),r=s.mtime instanceof Date?s.mtime:new Date(s.mtime),n=N._formatDateBadge(r,"smart"),l=N._formatFileSize(s.size,"auto");e.text=`$(clock) ${n} $(file) ${l}`,e.show()}catch(o){e.hide(),D.debug("Failed to update status bar",o)}},"updateStatusBar");return y.window.onDidChangeActiveTextEditor(t),y.window.onDidChangeTextEditorSelection(t),t(),a.subscriptions.push(e),e}d(Xt,"initializeStatusBar");async function Mi(a){try{D=bi(),ne=yi(),a.subscriptions.push(ne),D.info("Explorer Dates: Extension activated");let e=y.env.uiKind===y.UIKind.Web;await y.commands.executeCommand("setContext","explorerDates.gitFeaturesAvailable",!e);let t=y.workspace.getConfiguration("explorerDates"),o=t.get("enableWorkspaceTemplates",!0),i=t.get("enableReporting",!0),s=t.get("enableExtensionApi",!0);N=new vi;let r=y.window.registerFileDecorationProvider(N);a.subscriptions.push(r),a.subscriptions.push(N),a.subscriptions.push(D),await N.initializeAdvancedSystems(a);let n=null,l=null,c=null,u=null,h=d(()=>{if(!n){let{OnboardingManager:b}=qt();n=new b(a)}return n},"getOnboardingManager"),w=d(()=>{if(!o)throw new Error("Workspace templates are disabled via explorerDates.enableWorkspaceTemplates");if(!l){let{WorkspaceTemplatesManager:b}=Kt();l=new b(a)}return l},"getWorkspaceTemplatesManager"),p=d(()=>{if(!c){let{ExtensionApiManager:b}=Qt();c=new b,a.subscriptions.push(c)}return c},"getExtensionApiManager"),v=d(()=>{if(!i)throw new Error("Reporting is disabled via explorerDates.enableReporting");if(!u){let{ExportReportingManager:b}=Zt();u=new b,a.subscriptions.push(u)}return u},"getExportReportingManager"),L=d(()=>p().getApi(),"apiFactory");s?a.exports=L:(a.exports=void 0,D.info("Explorer Dates API exports disabled via explorerDates.enableExtensionApi")),y.workspace.getConfiguration("explorerDates").get("showWelcomeOnStartup",!0)&&await h().shouldShowOnboarding()&&setTimeout(()=>{h().showWelcomeMessage()},5e3),Ci({context:a,fileDateProvider:N,logger:D,l10n:ne}),xi({context:a,fileDateProvider:N,logger:D,generators:{generateWorkspaceActivityHTML:Fi,generatePerformanceAnalyticsHTML:$i,generateDiagnosticsHTML:Ti,generateDiagnosticsWebview:ki}}),Di({context:a,logger:D,getOnboardingManager:h});let be=y.commands.registerCommand("explorerDates.openTemplateManager",async()=>{try{if(!o){y.window.showInformationMessage("Workspace templates are disabled. Enable explorerDates.enableWorkspaceTemplates to use this command.");return}await w().showTemplateManager(),D.info("Template manager opened")}catch(b){D.error("Failed to open template manager",b),y.window.showErrorMessage(`Failed to open template manager: ${b.message}`)}});a.subscriptions.push(be);let H=y.commands.registerCommand("explorerDates.saveTemplate",async()=>{try{if(!o){y.window.showInformationMessage("Workspace templates are disabled. Enable explorerDates.enableWorkspaceTemplates to save templates.");return}let b=await y.window.showInputBox({prompt:"Enter template name",placeHolder:"e.g., My Project Setup"});if(b){let X=await y.window.showInputBox({prompt:"Enter description (optional)",placeHolder:"Brief description of this template"})||"";await w().saveCurrentConfiguration(b,X)}D.info("Template saved")}catch(b){D.error("Failed to save template",b),y.window.showErrorMessage(`Failed to save template: ${b.message}`)}});a.subscriptions.push(H);let ce=y.commands.registerCommand("explorerDates.generateReport",async()=>{try{if(!i){y.window.showInformationMessage("Reporting features are disabled. Enable explorerDates.enableReporting to generate reports.");return}await v().showReportDialog(),D.info("Report generation started")}catch(b){D.error("Failed to generate report",b),y.window.showErrorMessage(`Failed to generate report: ${b.message}`)}});a.subscriptions.push(ce);let ye=y.commands.registerCommand("explorerDates.showApiInfo",async()=>{try{if(!s){y.window.showInformationMessage("Explorer Dates API is disabled via settings.");return}let b=y.window.createWebviewPanel("apiInfo","Explorer Dates API Information",y.ViewColumn.One,{enableScripts:!0});b.webview.html=Si(L()),D.info("API information panel opened")}catch(b){D.error("Failed to show API information",b),y.window.showErrorMessage(`Failed to show API information: ${b.message}`)}});a.subscriptions.push(ye);let U,Z=y.workspace.getConfiguration("explorerDates"),_e=Z.get("performanceMode",!1);Z.get("showStatusBar",!1)&&!_e&&(U=Xt(a));let le=y.workspace.onDidChangeConfiguration(b=>{if(b.affectsConfiguration("explorerDates.showStatusBar")||b.affectsConfiguration("explorerDates.performanceMode")){let X=y.workspace.getConfiguration("explorerDates"),ee=X.get("showStatusBar",!1),de=X.get("performanceMode",!1),te=ee&&!de;te&&!U?U=Xt(a):!te&&U&&(U.dispose(),U=null)}});a.subscriptions.push(le),D.info("Explorer Dates: Date decorations ready")}catch(e){let t=`${ne?ne.getString("activationError"):"Explorer Dates failed to activate"}: ${e.message}`;throw D&&D.error("Extension activation failed",e),y.window.showErrorMessage(t),e}}d(Mi,"activate");async function Ei(){try{D&&D.info("Explorer Dates extension is being deactivated"),N&&typeof N.dispose=="function"&&await N.dispose(),D&&D.info("Explorer Dates extension deactivated successfully")}catch(a){D&&D.error("Explorer Dates: Error during deactivation",a)}}d(Ei,"deactivate");eo.exports={activate:Mi,deactivate:Ei}});var oo=to();module.exports={activate:oo.activate,deactivate:oo.deactivate};
