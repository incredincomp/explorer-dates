...
${history}`,{modal:!0})}),logger.info(`File history requested for: ${targetUri.fsPath||targetUri.path}`)}catch(error){logger.error("Failed to show file history",error),vscode.window.showErrorMessage(`Failed to show file history: ${error.message}`)}})),subscriptions.push(vscode.commands.registerCommand("explorerDates.compareWithPrevious",async uri=>{try{if(isWeb){vscode.window.showInformationMessage("Git comparisons are unavailable on VS Code for Web.");return}let targetUri=uri;if(!targetUri&&vscode.window.activeTextEditor&&(targetUri=vscode.window.activeTextEditor.document.uri),!targetUri){vscode.window.showWarningMessage("No file selected");return}if(!vscode.workspace.getWorkspaceFolder(targetUri)){vscode.window.showWarningMessage("File is not in a workspace");return}await vscode.commands.executeCommand("git.openChange",targetUri),logger.info(`Git diff opened for: ${targetUri.fsPath||targetUri.path}`)}catch(error){logger.error("Failed to compare with previous version",error),vscode.window.showErrorMessage(`Failed to compare with previous version: ${error.message}`)}})),subscriptions.push(vscode.commands.registerCommand("explorerDates.applyCustomColors",async()=>{try{let customColors=vscode.workspace.getConfiguration("explorerDates").get("customColors",{veryRecent:"#00ff00",recent:"#ffff00",old:"#ff0000"}),message=`To use custom colors with Explorer Dates, add the following to your settings.json:
