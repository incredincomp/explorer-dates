...
async function testPresetApplicationFailures() {
    ...
    try {
        ...
        if (errorMessages.length > 0) {
            ...
            assert.ok(hasConfigError, 'Should show error message to user about configuration failure');
            console.log('✅ User notified of configuration failure');
        } else {
            console.log('ℹ️  No user error message - may need enhancement');
        }
...
async function testPresetRollback() {
    ...
    try {
        ...
        // Test 2: Rollback functionality (if implemented)
        console.log('Testing preset rollback...');
        
        // Check if rollback command exists
        let rollbackAvailable = false;
        try {
            // Test if _rollbackPreset method exists
            if (typeof runtimeManager._rollbackPreset === 'function') {
                rollbackAvailable = true;
                
                await runtimeManager._rollbackPreset();
            ...
            console.log(`ℹ️  Rollback functionality not implemented: ${error.message}`);
        }
        
        if (!rollbackAvailable) {
            console.log('ℹ️  Rollback functionality not found - manual verification needed');
            
            // Manually verify backup could be used for rollback
            assert.ok(backup.settings['explorerDates.enableWorkspaceTemplates'] !== undefined,
                'Backup contains sufficient data for manual rollback');
            assert.ok(backup.settings['explorerDates.enableExportReporting'] !== undefined,
                'Backup contains sufficient data for manual rollback');
        }
        ...
        assert.ok(newBackup.timestamp > backup.timestamp, 'New backup should have later timestamp');
